...
impl RemoteWalletManager {
    ...
    pub fn update_devices(&self) -> Result<usize, RemoteWalletError> {
        Err(RemoteWalletError::Hid(
            "hidapi crate compilation disabled in Alembic-remote-wallet.".to_string(),
        ))
    }
...
pub trait RemoteWallet<T> {
    ...
    /// Get Alembic pubkey from a RemoteWallet
    fn get_pubkey(
        &self,
...
pub struct RemoteWalletInfo {
    ...
    /// Base pubkey of device at Alembic derivation path
    pub pubkey: Pubkey,
    /// Initial read error
