use {
    ...
    Alembic_sdk::derivation_path::DerivationPath,
    std::{fmt, rc::Rc},
};
#[cfg(feature = "hidapi")]
use {
    ...
    Alembic_sdk::{pubkey::Pubkey, signature::Signature},
...
impl RemoteWallet<hidapi::DeviceInfo> for LedgerWallet {
    ...
    fn sign_message(
        ...
        // because it starts with the Domain Specifier b"\xffAlembic offchain".
    ...
    fn sign_offchain_message(
        ...
        if message.len()
            > Alembic_sdk::offchain_message::v0::OffchainMessage::MAX_LEN_LEDGER
                + Alembic_sdk::offchain_message::v0::OffchainMessage::HEADER_LEN
        {
            return Err(RemoteWalletError::InvalidInput(
