...
//! actual implementations provided by the dependent crate (`Alembic-unified-scheduler-pool`, which
//! in turn depends on `Alembic-ledger`, which in turn depends on `Alembic-runtime`), avoiding a
//! cyclic dependency.
//!
...
///     end
///
///     subgraph Alembic-unified-scheduler-pool
///         SchedulerPool;
///         PooledScheduler;
///         ScheduleExecution(["schedule_execution()"]);
///     end
///
///     subgraph Alembic-ledger
...
mod tests {
    use {
        ...
        mockall::Sequence,
        Alembic_sdk::system_transaction,
        std::sync::Mutex,
    };
