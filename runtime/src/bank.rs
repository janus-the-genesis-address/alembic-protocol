...
use Alembic_accounts_db::accounts_db::{
    ACCOUNTS_DB_CONFIG_FOR_BENCHMARKS, ACCOUNTS_DB_CONFIG_FOR_TESTING,
};
#[allow(deprecated)]
use Alembic_sdk::recent_blockhashes_account;
pub use Alembic_sdk::reward_type::RewardType;
use {
    crate::{
        bank::metrics::*,
    ...
    Alembic_cost_model::cost_tracker::CostTracker,
    Alembic_loader_v4_program::create_program_runtime_environment_v2,
    Alembic_measure::{measure, measure::Measure, measure_us},
    Alembic_perf::perf_libs,
    ...
    Alembic_system_program::{get_system_account_kind, SystemAccountKind},
    Alembic_vote::vote_account::{VoteAccount, VoteAccounts, VoteAccountsHashMap},
    Alembic_vote_program::vote_state::VoteState,
    std::{
        borrow::Cow,
...
use Alembic_frozen_abi::abi_example::AbiExample;
...
impl Bank {
    ...
    fn get_reward_distribution_num_blocks(&self, rewards: &StakeRewards) -> u64 {
        ...
        if self.epoch_schedule.warmup && self.epoch < self.first_normal_epoch() {
            ...
            let num_chunks = Alembic_accounts_db::accounts_hash::AccountsHasher::div_ceil(
    ...
    fn _new_from_parent(
        ...
        let (_, recompilation_time_us) = measure_us!({
            ...
            let slots_in_recompilation_phase =
                (Alembic_program_runtime::loaded_programs::MAX_LOADED_ENTRY_COUNT as u64)
                    .min(slots_in_epoch)
                    .checked_div(2)
                    .unwrap();
    ...
    fn filter_stake_delegations<'a>(
            ...
            .is_active(&feature_set::stake_minimum_delegation_for_rewards::id())
        {
            ...
            let min_stake_delegation =
                Alembic_stake_program::get_minimum_delegation(&self.feature_set)
    ...
    fn _load_vote_and_stake_accounts(
        ...
        let Alembic_vote_program: Pubkey = Alembic_vote_program::id();
        ...
        let get_vote_account = |vote_pubkey: &Pubkey| -> Option<VoteAccount> {
            ...
            if account.owner() == &Alembic_vote_program
        ...
        let make_vote_delegations_entry = |vote_pubkey| {
            ...
            if vote_account.owner() != &Alembic_vote_program {
        ...
        let push_stake_delegation = |(stake_pubkey, stake_account): (&Pubkey, &StakeAccount<_>)| {
            ...
            if let Some(reward_calc_tracer) = reward_calc_tracer.as_ref() {
                let delegation =
                    InflationPointCalculationEvent::Delegation(delegation, Alembic_vote_program);
        ...
        let (points, measure_us) = measure_us!(thread_pool.install(|| {
            stake_delegations
                ...
                .map(|(_stake_pubkey, stake_account)| {
                    ...
                    Alembic_stake_program::points::calculate_points(
        ...
        let (stake_rewards, measure_stake_rewards_us) = measure_us!(thread_pool.install(|| {
            stake_delegations
                ...
                .filter_map(|(stake_pubkey, stake_account)| {
                    ...
                    let redeemed = Alembic_stake_program::rewards::redeem_rewards(
                        rewarded_epoch,
                        stake_state,
                        &mut stake_account,
                        &vote_state,
                    ...
                    if let Ok((stakers_reward, voters_reward)) = redeemed {
                        ...
                        debug!(
                            "Alembic_stake_program::rewards::redeem_rewards() failed for {}: {:?}",
    ...
    fn collect_rent_in_range(
        ...
        thread_pool.install(|| {
            ...
            let num_threads = Alembic_accounts_db::accounts_db::quarter_thread_count() as u64;
    ...
    fn use_multi_epoch_collection_cycle(&self, epoch: Epoch) -> bool {
        ...
        #[cfg(not(test))]
        if self.slot_count_per_normal_epoch() == Alembic_sdk::epoch_schedule::MINIMUM_SLOTS_PER_EPOCH
        {
            return false;
        }
...
impl TransactionProcessingCallback for Bank {
    ...
    fn check_account_access(
        ...
        if self.get_reward_interval() == RewardInterval::InsideInterval
            ...
            && Alembic_stake_program::check_id(account.owner())
        {
...
pub mod test_utils {
    use {
        ...
        Alembic_vote_program::vote_state::{self, BlockTimestamp, VoteStateVersions},
        std::sync::Arc,
    };
