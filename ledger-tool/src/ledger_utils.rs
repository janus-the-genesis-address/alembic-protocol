use {
    ...
    Alembic_geyser_plugin_manager::geyser_plugin_service::{
...
pub fn load_and_process_ledger(
    ...
    let account_paths = if let Some(account_paths) = arg_matches.value_of("account_paths") {
        // If this blockstore access is Primary, no other process (Alembic-validator) can hold
        // Primary access. So, allow a custom accounts path without worry of wiping the accounts
        // of Alembic-validator.
        if !blockstore.is_primary_access() {
            ...
            // the Primary access instead of holding it to give priority to Alembic-validator over
            // Alembic-ledger-tool should Alembic-validator start before we've finished.
            info!(
