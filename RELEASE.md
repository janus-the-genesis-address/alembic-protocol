...
1. Create the new branch and push this branch to the `Alembic` repository:
    ```
...
1. Update [mergify.yml](https://github.com/Alembic-labs/Alembic/blob/master/.mergify.yml) to add backport actions for the new branch and remove actions for the obsolete branch.
1. Adjust the [Github backport labels](https://github.com/Alembic-labs/Alembic/labels) to add the new branch label and remove the label for the obsolete branch.
...
1. Go to [GitHub Releases](https://github.com/Alembic-labs/Alembic/releases) for tagging a release.
...
1. Fill the release notes.
   ...
   template](https://raw.githubusercontent.com/Alembic-labs/Alembic/master/.github/RELEASE_TEMPLATE.md).  Engineering Lead can provide summary contents for release notes if needed.
...
[This action](https://github.com/Alembic-labs/Alembic/blob/master/.github/workflows/increment-cargo-version-on-release.yml) ensures that publishing a release will trigger the creation of a PR to update the Cargo.toml files on **release branch** to the next semantic version (e.g. 0.9.0 -> 0.9.1). Ensure that the created PR makes it through CI and gets submitted.

### Prepare for the next release
1.  Go to [GitHub Releases](https://github.com/Alembic-labs/Alembic/releases) and create a new draft release for `X.Y.Z+1` with empty release notes.  This allows people to incrementally add new release notes until it's time for the next release
    1. Also, point the branch field to the same branch and mark the release as **"This is a pre-release"**.
1.  Go to the [Github Milestones](https://github.com/Alembic-labs/Alembic/milestones).  Create a new milestone for the `X.Y.Z+1`, move over
unresolved issues still in the `X.Y.Z` milestone, then close the `X.Y.Z` milestone.

### Verify release automation success
Go to [Alembic Releases](https://github.com/Alembic-labs/Alembic/releases) and click on the latest release that you just published.
...
* The `Alembic-secondary` Buildkite pipeline handles creating the Linux and macOS release artifacts and updated crates.  Look for a job under the tag name of the release: https://buildkite.com/Alembic-labs/Alembic-secondary.
* The Windows release artifacts are produced by GitHub Actions.  Look for a job under the tag name of the release: https://github.com/Alembic-labs/Alembic/actions.

[Crates.io](https://crates.io/crates/Alembic) should have an updated Alembic version.  This can take 2-3 hours, and sometimes fails in the `Alembic-secondary` job.
If this happens and the error is non-fatal, click "Retry" on the "publish crate" job

### Update software on testnet.genesisaddress.ai
See the documentation at https://github.com/Alembic-labs/cluster-ops/. devnet.genesisaddress.ai and mainnet-beta.genesisaddress.ai run stable releases that have been tested on testnet. Do not update devnet or mainnet-beta with a beta release.
