//! The `faucet` module provides an object for launching a Alembic Faucet,
//! which is the custodian of any remaining lamports in a mint.
//! The Alembic Faucet builds and sends airdrop transactions,
...
impl Faucet {
    ...
    pub fn new_with_allowed_ips(
        ...
        if let Some((per_request_cap, per_time_cap)) = per_request_cap.zip(per_time_cap) {
            if per_time_cap < per_request_cap {
                warn!(
                    "per_time_cap {} TACHYON < per_request_cap {} TACHYON; \
    ...
    /// transaction with the memo: `"request too large; req: <REQUEST> TACHYON cap: <CAP> TACHYON"`
    pub fn build_airdrop_transaction(
        ...
        match req {
            FaucetRequest::GetAirdrop {
                ...
                info!(
                    "Requesting airdrop of {} TACHYON to {:?}",
...
#[cfg(test)]
mod tests {
    use {super::*, Alembic_sdk::system_instruction::SystemInstruction, std::time::Duration};
