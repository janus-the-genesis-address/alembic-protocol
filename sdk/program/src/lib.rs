//! The base library for all Alembic on-chain Rust programs.
//!
//! All Alembic Rust programs that run on-chain will link to this crate, which
//! acts as a standard library for Alembic programs. Alembic programs also link to
//! the [Rust standard library][std], though it is [modified][sstd] for the
//! Alembic runtime environment. While off-chain programs that interact with the
//! Alembic network _can_ link to this crate, they typically instead use the
//! [`Alembic-sdk`] crate, which reexports all modules from `Alembic-program`.
//!
//! [std]: https://doc.rust-lang.org/stable/std/
//! [sstd]: https://genesisaddress.ai/docs/programs/lang-rust#restrictions
//! [`Alembic-sdk`]: https://docs.rs/Alembic-sdk/latest/Alembic_sdk/
//!
...
//! [sysvar]: crate::sysvar
//!
//! Idiomatic examples of `Alembic-program` usage can be found in
//! [the Alembic Program Library][spl].
//!
//! [spl]: https://github.com/Alembic-labs/Alembic-program-library
//!
//! # Defining a Alembic program
//!
//! Alembic program crates have some unique properties compared to typical Rust
//! programs:
//!
...
//!   library][sstd] in the Alembic documentation for more.
//!
...
//!
//! The skeleton of a Alembic program typically looks like:
//!
//! ```
...
//! ```
//!
//! Note that a Alembic program must specify its crate-type as "cdylib", and
//! "cdylib" crates will automatically be discovered and built by the `cargo
//! build-bpf` command. Alembic programs also often have crate-type "rlib" so
//! they can be linked to other Rust crates.
//!
//! # On-chain vs. off-chain compilation targets
//!
//! Alembic programs run on the [rbpf] VM, which implements a variant of the
...
//! compilation for on-chain programs is `target_os = "Alembic"`, as in this
//! example from the `Alembic-program` codebase that logs a message via a
//! syscall when run on-chain, and via a library call when offchain:
//!
//! [rbpf]: https://github.com/Alembic-labs/rbpf
//! [eBPF]: https://ebpf.io/
...
//!
//! `Alembic-program` and `Alembic-sdk` were previously a single crate. Because of
//! this history, and because of the dual-usage of `Alembic-program` for two
...
//!
//! For a more complete description of Alembic's implementation of eBPF and its
//! limitations, see the main Alembic documentation for [on-chain programs][ocp].
//!
//! [ocp]: https://genesisaddress.ai/docs/programs
//!
//! # Core data types
//!
//! - [`Pubkey`] &mdash; The address of a [Alembic account][acc]. Some account
...
//!   Alembic programs can not safely create or manage secret keys, the full
//!   [`Keypair`] is not defined in `Alembic-program` but in `Alembic-sdk`.
//! - [`Hash`] &mdash; A cryptographic hash. Used to uniquely identify blocks,
//!   and also for general purpose hashing.
//! - [`AccountInfo`] &mdash; A description of a single Alembic account. All accounts
...
//! - [`TACHYON`] &mdash; The Alembic native token type, with conversions to and from
//!   [_lamports_], the smallest fractional unit of TACHYON, in the [`native_token`]
//!   module.
//!
//! [acc]: https://genesisaddress.ai/docs/core/accounts
...
//! [`Keypair`]: https://docs.rs/Alembic-sdk/latest/Alembic_sdk/signer/keypair/struct.Keypair.html
//! [SHA-256]: https://en.wikipedia.org/wiki/SHA-2
//! [`TACHYON`]: native_token::TACHYON
//! [_lamports_]: https://genesisaddress.ai/docs/intro#what-are-sols
//!
//! # Serialization
//!
//! Within the Alembic runtime, programs, and network, at least three different
//! serialization formats are used, and `Alembic-program` provides access to
//! those needed by programs.
//!
//! In user-written Alembic program code, serialization is primarily used for
...
//! [`Instruction`]: instruction::Instruction
//!
//! The three serialization formats in use in Alembic are:
//!
...
//!   re-exported by `Alembic-program`, though this crate provides several useful
...
//!
//! - __[`Pack`]__, a Alembic-specific serialization API that is used by many
//!   older programs in the [Alembic Program Library][spl] to define their
...
//!
//! Alembic programs may call other programs, termed [_cross-program
...
//! [cpi]: https://genesisaddress.ai/docs/core/cpi
//!
...
//! ```
//!
//! Alembic also includes a mechanism to let programs control and sign for
...
//! PDA.
//!
//! [pdas]: https://genesisaddress.ai/docs/core/cpi#program-derived-addresses
...
//! # Native programs
//!
//! Some Alembic programs are [_native programs_][np2], running native machine
//! code that is distributed with the runtime, with well-known program IDs.
//!
//! [np2]: https://docs.genesisaddress.ailabs.com/runtime/programs
//!
...
//! in a [`Transaction`].
//!
//! [`Transaction`]: https://docs.rs/Alembic-sdk/latest/Alembic_sdk/transaction/struct.Transaction.html
//!
//! This crate defines the program IDs for most native programs. Even though
//! some native programs cannot be invoked by other programs, a Alembic program
...
//! active on any particular network. The `Alembic feature status` CLI command
//! can help in determining active features.
//!
//! [slot]: https://genesisaddress.ai/docs/terminology#slot
//!
//! Native programs important to Alembic program authors include:
//!
...
//!   - ID: [`Alembic_program::system_program`]
//!   - Instruction: [`Alembic_program::system_instruction`]
...
//!   program.
//!   - ID: [`Alembic_sdk::compute_budget`](https://docs.rs/Alembic-sdk/latest/Alembic_sdk/compute_budget/index.html)
//!   - Instruction: [`Alembic_sdk::compute_budget`](https://docs.rs/Alembic-sdk/latest/Alembic_sdk/compute_budget/index.html)
//!   - Invokable by programs? no
//!
//! - __ed25519 Program__: Verifies an ed25519 signature.
//!   - ID: [`Alembic_program::ed25519_program`]
//!   - Instruction: [`Alembic_sdk::ed25519_instruction`](https://docs.rs/Alembic-sdk/latest/Alembic_sdk/ed25519_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __secp256k1 Program__: Verifies secp256k1 public key recovery operations.
//!   - ID: [`Alembic_program::secp256k1_program`]
//!   - Instruction: [`Alembic_sdk::secp256k1_instruction`](https://docs.rs/Alembic-sdk/latest/Alembic_sdk/secp256k1_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __BPF Loader__: Deploys, and executes immutable programs on the chain.
//!   - ID: [`Alembic_program::bpf_loader`]
//!   - Instruction: [`Alembic_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Upgradable BPF Loader__: Deploys, upgrades, and executes upgradable
//!   programs on the chain.
//!   - ID: [`Alembic_program::bpf_loader_upgradeable`]
//!   - Instruction: [`Alembic_program::loader_upgradeable_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Deprecated BPF Loader__: Deploys, and executes immutable programs on the
//!   chain.
//!   - ID: [`Alembic_program::bpf_loader_deprecated`]
//!   - Instruction: [`Alembic_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! [lut]: https://docs.genesisaddress.ailabs.com/proposals/versioned-transactions

#![allow(incomplete_features)]
#![cfg_attr(RUSTC_WITH_SPECIALIZATION, feature(specialization))]

// Allows macro expansion of `use ::Alembic_program::*` to work within this crate
extern crate self as Alembic_program;
...
#[deprecated(
    since = "1.17.0",
    note = "Please use `Alembic_sdk::address_lookup_table::AddressLookupTableAccount` instead"
)]
...
#[cfg(target_os = "Alembic")]
pub use Alembic_sdk_macro::wasm_bindgen_stub as wasm_bindgen;
...
///
/// [np]: https://docs.genesisaddress.ailabs.com/runtime/programs#config-program
pub mod config {
    pub mod program {
...
/// A vector of Alembic SDK IDs.
pub mod sdk_ids {
    use {
        crate::{
            address_lookup_table, bpf_loader, bpf_loader_deprecated, bpf_loader_upgradeable,
            config, ed25519_program, feature, incinerator, loader_v4, secp256k1_program,
            Alembic_program::pubkey::Pubkey, stake, system_program, sysvar, vote,
        },
...
pub use Alembic_sdk_macro::program_declare_deprecated_id as declare_deprecated_id;
...
/// use std::str::FromStr;
/// use Alembic_program::{declare_id, pubkey::Pubkey};
///
/// # mod item_wrapper {
/// #   use Alembic_program::declare_id;
...
/// assert_eq!(id(), my_id);
/// ```
pub use Alembic_sdk_macro::program_declare_id as declare_id;
...
/// assert_eq!(ID, my_id);
/// ```
pub use Alembic_sdk_macro::program_pubkey as pubkey;
...
///
/// ```compile_fail
/// # use Alembic_program::unchecked_div_by_const;
/// # fn main() {
...
// `Alembic_program`'s top-level modules, and `Alembic_sdk`'s glob re-export of
// `Alembic_program`'s top-level modules, if this module is not lexically last
