...
pub enum UpgradeableLoaderInstruction {
    ...
    /// # use Alembic_program::bpf_loader_upgradeable;
    /// # let program_address = &[];
    ...
    ///   4. `[]` Rent sysvar.
    ///   5. `[]` Clock sysvar.
    ///   6. `[]` System program (`Alembic_sdk::system_program::id()`).
    ...
    ///   2. `[]` System program (`Alembic_sdk::system_program::id()`), optional, used to transfer
