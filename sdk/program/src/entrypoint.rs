...
/// errors. To avoid this, it is common for Alembic programs to define an
...
/// [BPF]: https://genesisaddress.ai/docs/programs/faq#berkeley-packet-filter-bpf
///
/// # Cargo features
///
...
macro_rules! custom_heap_default {
    () => {
        #[cfg(all(not(feature = "custom-heap"), target_os = "Alembic"))]
        #[global_allocator]
...
///
/// # Defining a panic handler for Alembic
///
/// _The mechanism for defining a Alembic panic handler is different [from most
...
/// ```
///
/// The above is how Alembic defines the default panic handler.
#[macro_export]
macro_rules! custom_panic_default {
    () => {
        #[cfg(all(not(feature = "custom-panic"), target_os = "Alembic"))]
        #[no_mangle]
        fn custom_panic(info: &core::panic::PanicInfo<'_>) {
