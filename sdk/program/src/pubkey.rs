...
/// The address of a [Alembic account][acc].
///
/// Some account addresses are [ed25519] public keys, with corresponding secret
/// keys that are managed off-chain. Often, though, account addresses do not
/// have corresponding secret keys &mdash; as with [_program derived
/// addresses_][pdas] &mdash; or the secret key is not relevant to the operation
/// of a program, and may have even been disposed of. As running Alembic programs
/// can not safely create or manage secret keys, the full [`Keypair`] is not
/// defined in `Alembic-program` but in `Alembic-sdk`.
///
...
impl Pubkey {
    ...
    ///
    /// [pda]: https://genesisaddress.ai/docs/core/cpi#program-derived-addresses
    ///
    ...
    /// as a Alembic `Pubkey`, except they are ensured to not be on the ed25519
    ...
    ///
    /// As all account addresses accessed by an on-chain Alembic program must be
    ...
    /// program, which invokes an on-chain Alembic program that uses the address
    ...
    /// instruction as a Alembic transaction; and off-chain, as a library, so
    /// that clients can share the instruction data structure, constructors, and
    /// other common code.
    ///
    /// First the on-chain Alembic program:
    ///
    /// ```
    ...
    ///
    /// ```
    /// # use Alembic_program::pubkey::Pubkey;
    /// # let program_id = Pubkey::new_unique();
