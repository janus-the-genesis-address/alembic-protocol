...
fn test_Alembic_package(
    ...
    // Pass --sbf-out-dir along to the Alembic-program-test crate
...
fn test_Alembic(config: Config, manifest_path: Option<PathBuf>) {
    ...
    if let Some(root_package) = metadata.root_package() {
                    ...
                    .any(|p| root_package.id.repr.contains(p)))
        {
            ...
            test_Alembic_package(&config, metadata.target_directory.as_ref(), root_package);
            return;
        }
    }
    ...
    for package in all_sbf_packages {
        if config.packages.is_empty() || config.packages.iter().any(|p| package.id.repr.contains(p))
        {
            ...
            test_Alembic_package(&config, metadata.target_directory.as_ref(), package);
        }
    }
}
...
fn main() {
    ...
    let matches = clap::Command::new(crate_name!())
        ...
        .arg(
            Arg::new("workspace")
                ...
                .takes_value(false)
                .alias("all")
                .help("Test all Alembic packages in the workspace"),
        )
        .arg(
            Arg::new("jobs")
                .short('j')
    ...
    test_Alembic(config, manifest_path);
}
