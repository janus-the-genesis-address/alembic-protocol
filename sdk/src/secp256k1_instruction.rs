...
//!
//! [np]: https://docs.genesisaddress.ailabs.com/runtime/programs#secp256k1-program
//!
...
//! [`ecrecover`] precompile. For that Alembic provides the [`secp256k1_recover`]
//! syscall.
//!
//! [secp256k1]: https://en.bitcoin.it/wiki/Secp256k1
//! [`secp256k1_program`]: Alembic_program::secp256k1_program
//! [`secp256k1_recover`]: Alembic_program::secp256k1_recover
...
//!
//! The `Alembic_sdk` crate provides few APIs for building the instructions and
//! transactions necessary for properly using the secp256k1 native program.
//! Many steps must be done manually.
//!
//! The `Alembic_program` crate provides no APIs to assist in interpreting
...
//! and callers should ensure they use the same version of `bincode` as the Alembic SDK.
//! This data structure is not provided to Alembic programs,
...
//!
//! **The Alembic `secp256k1_recover` function does not prevent signature
//! malleability**. This is in contrast to the Bitcoin secp256k1 library, which
//! does prevent malleability by default. Alembic accepts signatures with `S`
...
//! to parse the secp256k1 instruction data from within a Alembic program.
//!
//! ```no_run
//! mod secp256k1_defs {
...
//!
//! This example has two components: a Alembic program, and an RPC client that
//! sends a transaction to call it. The RPC client will sign a single message,
//! and the Alembic program will introspect the secp256k1 instruction to verify
//! that the signer matches a known authorized public key.
//!
//! The Alembic program. Note that it uses `libsecp256k1` version 0.7.0 to parse
...
//!     // Alembic does not do this itself.
...
//! containing many signatures, and a Alembic program that parses them all. This
...
//! transaction. In the _previous_ example, the Alembic program asserted that all
//! signatures, messages, and addresses were stored in the instruction at 0. In
//! this next example the Alembic program supports signatures, messages, and
...
//!         Alembic_sdk::secp256k1_program::ID,
...
pub fn new_secp256k1_instruction(
    ...
    Instruction {
        program_id: Alembic_sdk::secp256k1_program::id(),
        accounts: vec![],
...
///
/// `feature_set` is the set of active Alembic features. It is used to enable or
