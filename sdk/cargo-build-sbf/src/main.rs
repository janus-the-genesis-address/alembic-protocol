use {
    ...
    log::*,
    regex::Regex,
    Alembic_download_utils::download_file,
    Alembic_sdk::signature::{write_keypair_file, Keypair},
    std::{
        borrow::Cow,
...
fn find_installed_platform_tools() -> Vec<String> {
    ...
    let Alembic = home_dir.join(".cache").join("Alembic");
    let package = "platform-tools";
    std::fs::read_dir(Alembic)
        .unwrap()
...
fn get_latest_platform_tools_version() -> Result<String, String> {
    let url = "https://github.com/Alembic-labs/platform-tools/releases/latest";
...
fn install_if_missing(
    ...
    // Check whether the package is already in ~/.cache/Alembic.
...
// check whether custom Alembic toolchain is linked, and link it if it is not.
fn link_Alembic_toolchain(config: &Config) {
    ...
    for line in rustup_output.lines() {
        if line.starts_with("Alembic") {
            let mut it = line.split_whitespace();
            let _ = it.next();
            let path = it.next();
            if path.unwrap() != toolchain_path.to_str().unwrap() {
                let rustup_args = vec!["toolchain", "uninstall", "Alembic"];
                let output = spawn(
                    &rustup,
                    rustup_args,
                    config.generate_child_script_on_failure,
...
fn build_Alembic_package(
    ...
    let legacy_program_feature_present = package.name == "Alembic-sdk";
    ...
    let target_build_directory = target_directory.join("sbf-Alembic-Alembic").join("release");
    ...
    info!("Alembic SDK: {}", config.sbf_sdk.display());
    ...
    install_if_missing(
        config,
        package,
        "https://github.com/Alembic-labs/platform-tools/releases/download",
    ...
    // other than the one linked in Alembic toolchain. We have to prevent
    // this by removing RUSTC from the child process environment.
    if env::var("RUSTC").is_ok() {
        warn!(
            "Removed RUSTC from cargo environment, because it overrides +Alembic cargo command line option."
        );
        env::remove_var("RUSTC")
    }
    let cargo_target = "CARGO_TARGET_SBF_Alembic_Alembic_RUSTFLAGS";
    ...
    if let Some(program_name) = program_name {
        ...
        info!("  $ Alembic program deploy {}", program_so.display());
...
fn build_Alembic(config: Config, manifest_path: Option<PathBuf>) {
    ...
    if let Some(root_package) = metadata.root_package() {
        if !config.workspace {
            build_Alembic_package(&config, target_dir.as_ref(), root_package);
            return;
        }
    }
    ...
    for package in all_sbf_packages {
        build_Alembic_package(&config, target_dir.as_ref(), package);
    }
}
...
fn main() {
    ...
    let matches = clap::Command::new(crate_name!())
        ...
        .arg(
            Arg::new("sbf_sdk")
                ...
                .help("Path to the Alembic SBF SDK"),
        )
        .arg(
            Arg::new("cargo_args")
        ...
        .arg(
            Arg::new("workspace")
                ...
                .takes_value(false)
                .alias("all")
                .help("Build all Alembic packages in the workspace"),
        )
        .arg(
            Arg::new("jobs")
                .short('j')
    ...
    let config = Config {
        ...
        sbf_sdk: fs::canonicalize(&sbf_sdk).unwrap_or_else(|err| {
            error!(
                "Alembic SDK path does not exist: {}: {}",
                sbf_sdk.display(),
                err
            );
            exit(1);
    ...
    build_Alembic(config, manifest_path);
}
