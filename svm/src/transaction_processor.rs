...
impl<FG: ForkGraph> TransactionBatchProcessor<FG> {
    ...
    pub fn program_modification_slot<CB: TransactionProcessingCallback>(
        ...
        if bpf_loader_upgradeable::check_id(program.owner()) {
            ...
            let state = Alembic_loader_v4_program::get_state(program.data())
    ...
    fn load_program_accounts<CB: TransactionProcessingCallback>(
        ...
        debug_assert!(Alembic_bpf_loader_program::check_loader_id(
            program_account.owner()
        ));

        if loader_v4::check_id(program_account.owner()) {
            return Alembic_loader_v4_program::get_state(program_account.data())
                .ok()
                .and_then(|state| {
...
mod tests {
    use {
        super::*,
        Alembic_program_runtime::{
            loaded_programs::BlockRelation, Alembic_rbpf::program::BuiltinProgram,
        },
        Alembic_sdk::{
