...
cat >/etc/apt/apt.conf.d/99-Alembic <<'EOF'
...
"$Alembic_ROOT"/net/scripts/install-docker.sh
usermod -aG docker "$SETUP_USER"
"$Alembic_ROOT"/net/scripts/install-certbot.sh
...
"$Alembic_ROOT"/net/scripts/install-earlyoom.sh
"$Alembic_ROOT"/net/scripts/localtime.sh
"$Alembic_ROOT"/net/scripts/install-rsync.sh
"$Alembic_ROOT"/net/scripts/install-libssl-compatability.sh
