...
startNodes() {
  ...
  for i in $(seq 0 $((${#nodes[@]} - 1))); do
    ...
    if [[ "$i" -eq 1 ]]; then
      ...
      (
        set -x
        $Alembic_cli --keypair config/bootstrap-validator/identity.json \
...
killNodes() {
  ...
  echo "--- RPC exit"
  $Alembic_validator --ledger "$Alembic_CONFIG_DIR"/bootstrap-validator exit --force || true
  $Alembic_validator --ledger "$Alembic_CONFIG_DIR"/validator exit --force || true
...
verifyLedger() {
  for ledger in bootstrap-validator validator; do
    ...
    (
      set -x
      $Alembic_ledger_tool --ledger "$Alembic_CONFIG_DIR"/$ledger verify
    ) || flag_error
  done
}
...
while [[ $iteration -le $iterations ]]; do
  ...
  (
    ...
    $Alembic_keygen new --no-passphrase -fso $client_keypair || exit $?
    $Alembic_gossip --allow-private-addr spy -n 127.0.0.1:8001 --num-nodes-exactly $numNodes || exit $?
