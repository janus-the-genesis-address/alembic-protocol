...
case $testName in
test-stable)
  _ ci/intercept.sh cargo test --jobs "$JOBS" --all --tests --exclude Alembic-local-cluster ${V:+--verbose} -- --nocapture
  ;;
test-stable-sbf)
  ...
  # Alembic-keygen required when building C programs
  ...
  # SBF Alembic-sdk legacy compile test
  ...
  # expected number of Alembic-program crate compilations is 4. There
  # should be 3 builds of Alembic-program while 128bit crate is
  ...
  # Alembic-program is performed when simulation crate is built. This
  # last compiled Alembic-program is of different version, normally the
  # latest mainbeta release version.
  Alembic_program_count=$(grep -c 'Alembic-program v' cargo.log)
  rm -f cargo.log
  if ((Alembic_program_count > 20)); then
      echo "Regression of build redundancy ${Alembic_program_count}."
      echo "Review dependency features that trigger redundant rebuilds of Alembic-program."
      exit 1
  fi
  ...
  tar cjvf "$sbf_dump_archive" "${sbf_target_path}"/{deploy/*.txt,sbf-Alembic-Alembic/release/*.so}
  exit 0
  ;;
test-stable-perf)
  if [[ $(uname) = Linux ]]; then
    ...
    # Force CUDA for Alembic-core unit tests
  ...
  _ cargo test --package Alembic-perf --package Alembic-ledger --package Alembic-core --lib ${V:+--verbose} -- --nocapture
...
test-wasm)
  ...
  ;;
test-docs)
  _ cargo test --jobs "$JOBS" --all --doc --exclude Alembic-local-cluster ${V:+--verbose} -- --nocapture
  exit 0
  ;;
*)
...
(
  ...
  export RUST_LOG="Alembic_metrics=warn,info,$RUST_LOG"
