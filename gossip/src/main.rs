...
fn parse_gossip_host(matches: &ArgMatches, entrypoint_addr: Option<SocketAddr>) -> IpAddr {
    matches
        ...
        .map(|gossip_host| {
            Alembic_net_utils::parse_host(gossip_host).unwrap_or_else(|e| {
        ...
        .unwrap_or_else(|| {
            if let Some(entrypoint_addr) = entrypoint_addr {
                Alembic_net_utils::get_public_ip_addr(&entrypoint_addr).unwrap_or_else(|err| {
...
fn process_spy(matches: &ArgMatches, socket_addr_space: SocketAddrSpace) -> std::io::Result<()> {
    ...
    let gossip_addr = SocketAddr::new(
        ...
        value_t!(matches, "gossip_port", u16).unwrap_or_else(|_| {
            Alembic_net_utils::find_available_port_in_range(
