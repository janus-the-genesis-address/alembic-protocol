...
use {
    ...
    Alembic_perf::recycler::Recycler,
...
pub fn discover(
    ...
    let _ip_echo_server = ip_echo
        .map(|tcp_listener| Alembic_net_utils::ip_echo_server(tcp_listener, Some(my_shred_version)));
...
mod tests {
    ...
    fn test_gossip_services_spy() {
        ...
        let peer0 = Alembic_sdk::pubkey::new_rand();
        let peer1 = Alembic_sdk::pubkey::new_rand();
        ...
        let (met_criteria, _, _, _) = spy(
            ...
            None,
            TIMEOUT,
            Some(&[Alembic_sdk::pubkey::new_rand()]),
            None,
        );
        assert!(!met_criteria);
