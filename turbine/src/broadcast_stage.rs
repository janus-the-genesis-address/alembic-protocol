...
use {
    ...
    Alembic_ledger::{blockstore::Blockstore, shred::Shred},
    Alembic_measure::measure::Measure,
    Alembic_metrics::{inc_new_counter_error, inc_new_counter_info},
    Alembic_poh::poh_recorder::WorkingBankEntry,
...
pub enum Error {
    #[error(transparent)]
    Blockstore(#[from] Alembic_ledger::blockstore::BlockstoreError),
    #[error(transparent)]
    ClusterInfo(#[from] Alembic_gossip::cluster_info::ClusterInfoError),
    ...
    #[error(transparent)]
    TransportError(#[from] Alembic_sdk::transport::TransportError),
    ...
    fn new(
        ...
        thread_hdls.extend(socks.into_iter().map(|sock| {
            ...
            let run_transmit = move || loop {
                ...
                let res = Self::handle_error(res, "Alembic-broadcaster-transmit");
                if let Some(res) = res {
                    return res;
                }
            };
        ...
        thread_hdls.extend(
            repeat_with(|| {
                ...
                let run_record = move || loop {
                    ...
                    let res = Self::handle_error(res, "Alembic-broadcaster-record");
                    if let Some(res) = res {
                        return res;
                    }
                };
        ...
        let retransmit_thread = Builder::new()
            ...
            .spawn(move || loop {
                if let Some(res) = Self::handle_error(
                    ...
                    "Alembic-broadcaster-retransmit-check_retransmit_signals",
                ) {
                    return res;
                }
            })
