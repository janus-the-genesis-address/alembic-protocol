use {
    ...
    Alembic_account_decoder::parse_token::{real_number_string_trimmed, UiTokenAmount},
    Alembic_sdk::{
        hash::Hash,
...
pub mod generated {
    include!(concat!(
        env!("OUT_DIR"),
        "/Alembic.storage.confirmed_block.rs"
    ));
}
...
pub mod tx_by_addr {
    include!(concat!(
        env!("OUT_DIR"),
        "/Alembic.storage.transaction_by_addr.rs"
    ));
}

#[allow(clippy::derive_partial_eq_without_eq)]
pub mod entries {
    include!(concat!(env!("OUT_DIR"), "/Alembic.storage.entries.rs"));
}
