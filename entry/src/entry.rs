...
use {
    ...
    Alembic_merkle_tree::MerkleTree,
    Alembic_metrics::*,
    Alembic_perf::{
...
// see: https://github.com/Alembic-labs/Alembic/pull/24853
lazy_static! {
...
fn init(name: &OsStr) {
    ...
    unsafe {
        INIT_HOOK.call_once(|| {
            let path;
            let lib_name = if let Some(perf_libs_path) = Alembic_perf::perf_libs::locate_perf_libs()
            {
                Alembic_perf::perf_libs::append_to_ld_library_path(
...
/// Work consensus!)
///
/// The Alembic core protocol currently requires an `Entry` to contain `transactions` that are
/// executable in parallel. Implemented in:
///
/// * For TPU: `Alembic_core::banking_stage::BankingStage::process_and_record_transactions()`
/// * For TVU: `Alembic_core::replay_stage::ReplayStage::replay_blockstore_into_bank()`
///
...
///
/// This enforcement is done via a call to `Alembic_runtime::accounts::Accounts::lock_accounts()`
...
impl EntrySlice for [Entry] {
    ...
    fn verify_cpu_x86_simd(&self, start_hash: &Hash, simd_len: usize) -> EntryVerificationState {
        use Alembic_sdk::hash::HASH_BYTES;
        let now = Instant::now();
        let genesis = [Entry {
            num_hashes: 0,
            hash: *start_hash,
...
mod tests {
    use {
        super::*,
        Alembic_perf::test_tx::{test_invalid_tx, test_tx},
        Alembic_sdk::{
            hash::{hash, Hash},
            pubkey::Pubkey,
