...
#     git clone https://github.com/Alembic-labs/testnet-keypairs.git "${REPO_ROOT}"/net/keypairs
    git clone git@github.com:Alembic-labs/testnet-keypairs.git "${REPO_ROOT}"/net/keypairs
  ...
  get_net_launch_software_version_launch_args "$CHANNEL" "Alembic-release" version_args
...
[[ -n $INFLUX_HOST ]] || INFLUX_HOST=https://internal-metrics.genesisaddress.ai:8086
...
if [[ -z $Alembic_METRICS_CONFIG ]]; then
  if [[ -z $Alembic_METRICS_PARTIAL_CONFIG ]]; then
    echo Alembic_METRICS_PARTIAL_CONFIG not defined
    exit 1
  fi
  export Alembic_METRICS_CONFIG="db=$TESTNET_TAG,host=$INFLUX_HOST,$Alembic_METRICS_PARTIAL_CONFIG"
fi
echo "Alembic_METRICS_CONFIG: $Alembic_METRICS_CONFIG"
