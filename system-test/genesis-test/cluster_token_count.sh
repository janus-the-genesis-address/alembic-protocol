...
usage: $0 [cluster_rpc_url]
 ...
 Required arguments:
   cluster_rpc_url  - RPC URL and port for a running Alembic cluster (ex: http://34.83.146.144:8899)
EOF
  exit $exitcode
}

function get_cluster_version {
  clusterVersion="$(curl -s -X POST -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","id":1, "method":"getVersion"}' "$url" | jq '.result | ."Alembic-core" ')"
...
function get_token_capitalization {
  ...
  printf "Total token capitalization %'d TACHYON\n" "$totalSupplySol"
...
function get_program_account_balance_totals {
  ...
  printf "Total token balance in all %s accounts: %'d TACHYON\n" "$PROGRAM_NAME" "$totalAccountBalancesSol"
...
function sum_account_balances_totals {
  ...
  printf "Total TACHYON in all Account Balances: %'d\n" "$grandTotalAccountBalancesSol"
