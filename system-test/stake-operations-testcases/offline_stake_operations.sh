...
if [[ -z "$url" ]]; then
  echo Provide complete URL, ex: "$0" http://api.devnet.genesisaddress.ai:8899
  exit 1
fi
Alembic config set --url $url
...
Alembic-keygen new -o "$dummy_keypair" --no-passphrase --force --silent
Alembic config set --keypair $dummy_keypair
...
Alembic-keygen new -o "$online_system_account_keypair" --no-passphrase --force --silent
Alembic-keygen new -o "$online_nonce_account_keypair" --no-passphrase --force --silent

online_system_account_pubkey="$(Alembic-keygen pubkey $online_system_account_keypair)"
nonce_account_pubkey="$(Alembic-keygen pubkey $online_nonce_account_keypair)"
...
Alembic-keygen new -o "$offline_system_account_keypair" --no-passphrase --force --silent
Alembic-keygen new -o "$offline_staker_keypair" --no-passphrase --force --silent
Alembic-keygen new -o "$offline_withdrawer_keypair" --no-passphrase --force --silent
Alembic-keygen new -o "$offline_custodian_keypair" --no-passphrase --force --silent

offline_system_account_pubkey="$(Alembic-keygen pubkey $offline_system_account_keypair)"
offline_withdrawer_pubkey="$(Alembic-keygen pubkey $offline_withdrawer_keypair)"
offline_staker_pubkey="$(Alembic-keygen pubkey $offline_staker_keypair)"
offline_custodian_pubkey="$(Alembic-keygen pubkey $offline_custodian_keypair)"

# Airdrop some funds to the offline account.
Alembic airdrop 100 $offline_system_account_pubkey
Alembic airdrop 2 $online_system_account_pubkey

# Create a nonce account funded by the online account, with the authority given to the offline account
Alembic create-nonce-account $online_nonce_account_keypair 1 --nonce-authority $offline_system_account_pubkey --keypair $online_system_account_keypair
nonce="$(Alembic nonce $nonce_account_pubkey)"

execution_step OFFLINE SYSTEM ACCOUNT BALANCE BEFORE CREATING STAKE ACCOUNTS
(
  set -x
  Alembic balance $offline_system_account_pubkey
)
...
Alembic-keygen new -o "$stake_account_keypair" --no-passphrase --force --silent
stake_account_address="$(Alembic-keygen pubkey $stake_account_keypair)"

sign_only="$(Alembic create-stake-account $stake_account_keypair 50 \
...
Alembic create-stake-account $stake_account_keypair 50 \
...
Alembic-keygen new -o $split_stake_account_keypair --no-passphrase --force --silent
split_stake_account_address=$(Alembic-keygen pubkey $split_stake_account_keypair)

nonce="$(Alembic nonce $nonce_account_pubkey)"

sign_only="$(Alembic split-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
Alembic split-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
(
  set -x
  Alembic stake-account $split_stake_account_address
)
...
sign_only="$(Alembic stake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
  $split_stake_account_address --custodian $offline_custodian_keypair --lockup-epoch 0 \
  --keypair $offline_system_account_keypair --sign-only --url http://0.0.0.0)"

signers="$(get_signers_string "${sign_only[@]}")"

Alembic stake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
sign_only="$(Alembic withdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
Alembic withdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
  $split_stake_account_address $offline_system_account_pubkey 10 \
...
vote_account_pubkey="$(awk '{if(NR==4) print $2}'<<<"$(Alembic show-validators)")"
nonce="$(Alembic nonce $nonce_account_pubkey)"

# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(Alembic delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
Alembic delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(Alembic deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
# Send the signed transaction on the cluster
Alembic deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
