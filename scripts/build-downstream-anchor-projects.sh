#!/usr/bin/env bash
#
# Builds known downstream projects against local Alembic source
#
...
Alembic_dir=$PWD
cargo="$Alembic_dir"/cargo
cargo_build_sbf="$Alembic_dir"/cargo-build-sbf
cargo_test_sbf="$Alembic_dir"/cargo-test-sbf
...
anchor() {
  ...
  cp "$Alembic_dir"/rust-toolchain.toml .
  ...
  $cargo test
  # serum_dex and mpl-token-metadata are using caret versions of Alembic and SPL dependencies
  ...
  cd "$Alembic_dir"/target/downstream-projects-anchor
}
...
mango() {
  (
    ...
    cd mango-v4
    update_Alembic_dependencies . "$Alembic_ver"
    patch_crates_io_Alembic_no_header Cargo.toml "$Alembic_dir"
    $cargo_test_sbf --features enable-gpl
  )
}
...
metaplex() {
  (
    ...
    # copy toolchain file to use Alembic's rust version
    cp "$Alembic_dir"/rust-toolchain.toml mpl-token-metadata/
