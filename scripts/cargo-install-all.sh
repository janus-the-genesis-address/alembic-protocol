...
Alembic_ROOT="$("${readlink_cmd}" -f "${here}/..")"
cargo="${Alembic_ROOT}/cargo"
...
if [[ $CI_OS_NAME = windows ]]; then
  ...
  #XXX: Ensure `Alembic-genesis` is built LAST!
  # See https://github.com/Alembic-labs/Alembic/issues/5826
  BINS+=(Alembic-genesis)
fi
...
(
  ...
  if [[ -z "$validatorOnly" ]]; then
    ...
    source "$Alembic_ROOT"/scripts/spl-token-cli-version.sh

    # the patch-related configs are needed for rust 1.69+ on Windows; see Cargo.toml
    # shellcheck disable=SC2086 # Don't want to double quote $rust_version
    "$cargo" $maybeRustVersion \
      --config 'patch.crates-io.ntapi.git="https://github.com/Alembic-labs/ntapi"' \
...
(
  ...
  shopt -s nullglob
  for dep in target/"$buildProfile"/deps/libAlembic*program.*; do
