...
FLAGS:
        ...
        --no-Alembic-prefix    Do not require \`Alembic-\` prefix on PACKAGE_NAME
...
if ${require_Alembic_prefix} && ! [[ "${package_name}" =~ ^Alembic- ]]; then
  # shellcheck disable=SC2016 # backticks are not a command here
  echo 'error: PACKAGE_NAME MUST start with `Alembic-`' 1>&2
  display_help
  exit 1
fi
