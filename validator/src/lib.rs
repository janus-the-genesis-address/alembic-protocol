...
pub use Alembic_test_validator as test_validator;
use {
    console::style,
...
pub fn redirect_stderr_to_file(logfile: Option<String>) -> Option<JoinHandle<()>> {
    ...
    let filter = "Alembic=info";
    match logfile {
        None => {
            Alembic_logger::setup_with_default(filter);
            None
        }
        Some(logfile) => {
            #[cfg(unix)]
            ...
            #[cfg(not(unix))]
            {
                ...
                Alembic_logger::setup_file_with_default(&logfile, filter);
                None
            }
        }
    }
