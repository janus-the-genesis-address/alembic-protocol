use {
    ...
    itertools::Itertools,
    log::*,
    Alembic_accounts_db::accounts_index::{AccountIndex, AccountSecondaryIndexes},
    Alembic_clap_utils::{
        input_parsers::{pubkey_of, pubkeys_of, value_of},
        input_validators::normalize_to_url_if_moniker,
    },
    Alembic_core::consensus::tower_storage::FileTowerStorage,
    Alembic_faucet::faucet::run_local_faucet_with_port,
    Alembic_rpc::{
...
fn main() {
    ...
    let version = Alembic_version::version!();
    ...
    Alembic_runtime::snapshot_utils::remove_tmp_snapshot_archives(&ledger_path);
    ...
    let dynamic_port_range = matches.value_of("dynamic_port_range").map(|port_range| {
        Alembic_net_utils::parse_port_range(port_range).unwrap_or_else(|| {
            eprintln!("Failed to parse --dynamic-port-range");
            exit(1);
        })
    });
    let bind_address = matches.value_of("bind_address").map(|bind_address| {
        Alembic_net_utils::parse_host(bind_address).unwrap_or_else(|err| {
    ...
    if TestValidatorGenesis::ledger_exists(&ledger_path) {
        ...
        println_name_value(
            "\nNotice!",
            "No wallet available. `Alembic airdrop` localnet TACHYON after creating one\n",
        );
    }
