...
//!
//! This crate implements 3 Alembic-runtime traits (`InstalledScheduler`, `UninstalledScheduler` and
...
//! `Alembic-ledger`'s helper function called `execute_batch()`.
...
use {
    ...
    log::*,
    Alembic_ledger::blockstore_processor::{
    ...
    Alembic_sdk::transaction::{Result, SanitizedTransaction},
    Alembic_unified_scheduler_logic::Task,
...
// SchedulerPool must be accessed as a dyn trait from Alembic-runtime, because SchedulerPool
// contains some internal fields, whose types aren't available in Alembic-runtime (currently
...
mod tests {
    ...
    fn test_scheduler_schedule_execution_failure() {
        ...
        assert_matches!(
            ...
            Some((
                Err(Alembic_sdk::transaction::TransactionError::AccountNotFound),
                _timings
            ))
        );
    }
