...
declare -r Alembic_LOCK_FILE="/home/Alembic/.Alembic.lock"
...
colo_instance_run() {
  ...
  declare OUT
  set +e
  OUT=$(ssh -l Alembic -o "StrictHostKeyChecking=no" -o "ConnectTimeout=3" -n "${IP}" "${CMD}" 2>&1)
  declare RC=$?
  set -e
  while read -r LINE; do
...
colo_whoami() {
  ...
  done < <(colo_instance_run_foreach "[ -n \"\${Alembic_USER}\" ] && echo -e \"\${Alembic_USER}\\x1fEOL\"")
  echo "${ME}"
}

COLO_Alembic_USER=""
colo_get_Alembic_user() {
  if [ -z "${COLO_Alembic_USER}" ]; then
    COLO_Alembic_USER=$(colo_whoami)
  fi
  echo "${COLO_Alembic_USER}"
}
...
__colo_node_status_script() {
  ...
  exec 2>/dev/null  # Suppress stderr as the next call to exec fails most of
                    # the time due to ${Alembic_LOCK_FILE} not existing and is running from a
                    # subshell where normal redirection doesn't work
  exec 9<"${Alembic_LOCK_FILE}" && flock -s 9 && . "${Alembic_LOCK_FILE}" && exec 9>&-
  echo -e "\${Alembic_LOCK_USER}\\x1f\${Alembic_LOCK_INSTANCENAME}\\x1f\${PREEMPTIBLE}\\x1fEOL"
...
SSH_AUTHORIZED_KEYS='$("${__colo_here}"/add-datacenter-Alembic-user-authorized_keys.sh 2> /dev/null)'
