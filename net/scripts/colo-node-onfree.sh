...
if [[ -f "${Alembic_LOCK_FILE}" ]]; then
  ...
  . "${Alembic_LOCK_FILE}"
  if [[ "${Alembic_LOCK_USER}" = "${Alembic_USER}"  || -n "${FORCE_DELETE}" ]]; then
    ...
    git clean -qxdff
    rm -f /Alembic-scratch/* /Alembic-scratch/.[^.]*
...
EOAK
    ...
    echo "Invalid user: expected \"${Alembic_LOCK_USER}\" got \"${Alembic_USER}\"" 1>&2
  fi
  exec 9>&-
fi
${RC}
