...
cat >> ~/Alembic/on-reboot <<EOF
  ...
  (
    sudo Alembic_METRICS_CONFIG="$Alembic_METRICS_CONFIG" scripts/oom-monitor.sh
  ) > oom-monitor.log 2>&1 &
  ...
  if ${GPU_CUDA_OK} && [[ -e /dev/nvidia0 ]]; then
    echo Selecting Alembic-validator-cuda
    export Alembic_CUDA=1
  elif ${GPU_FAIL_IF_NONE} ; then
    echo "Expected GPU, found none!"
    export Alembic_GPU_MISSING=1
  fi
EOF
  ...
  bootstrap-validator)
    ...
    if [[ $skipSetup != true ]]; then
      clear_config_dir "$Alembic_CONFIG_DIR"
      ...
      setupValidatorKeypair() {
        ...
        if [[ -f net/keypairs/"$name".json ]]; then
          ...
          Alembic-keygen new --no-passphrase -so config/"$name".json
          if [[ "$name" =~ ^validator-identity- ]]; then
            name="${name//-identity-/-vote-}"
            Alembic-keygen new --no-passphrase -so config/"$name".json
            name="${name//-vote-/-stake-}"
        ...
        if [[ -n $internalNodesLamports ]]; then
          declare pubkey
          pubkey="$(Alembic-keygen pubkey config/"$name".json)"
...
EOF
      ...
      for i in $(seq 0 $((numBenchTpsClients-1))); do
        ...
        Alembic-bench-tps --write-client-keys config/bench-tps"$i".yml \
      ...
      if [[ "$extraPrimordialStakes" -gt 0 ]]; then
        ...
        for i in $(seq "$extraPrimordialStakes"); do
          args+=(--bootstrap-validator "$(Alembic-keygen pubkey "config/validator-identity-$i.json")"
                                       "$(Alembic-keygen pubkey "config/validator-vote-$i.json")"
                                       "$(Alembic-keygen pubkey "config/validator-stake-$i.json")"
          )
        done
      fi
      ...
      if [[ -n "$maybeWarpSlot" ]]; then
        ...
        Alembic-ledger-tool -l config/bootstrap-validator create-snapshot 0 config/bootstrap-validator $maybeWarpSlot
      fi

      Alembic-ledger-tool -l config/bootstrap-validator shred-version --max-genesis-archive-unpacked-size 1073741824 | tee config/shred-version

      if [[ -n "$maybeWaitForSupermajority" ]]; then
        bankHash=$(Alembic-ledger-tool -l config/bootstrap-validator bank-hash --halt-at-slot 0)
        shredVersion="$(cat "$Alembic_CONFIG_DIR"/shred-version)"
    ...
    if [[ "$tmpfsAccounts" = "true" ]]; then
      args+=(--accounts /mnt/Alembic-accounts)
    fi
...
EOF
  ...
  validator|blockstreamer)
    ...
    if [[ $skipSetup != true ]]; then
      ...
      if [[ $nodeType = blockstreamer ]]; then
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/Alembic/config/blockstreamer-identity.json "$Alembic_CONFIG_DIR"/validator-identity.json
      else
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/Alembic/config/validator-identity-"$nodeIndex".json "$Alembic_CONFIG_DIR"/validator-identity.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/Alembic/config/validator-stake-"$nodeIndex".json "$Alembic_CONFIG_DIR"/stake-account.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/Alembic/config/validator-vote-"$nodeIndex".json "$Alembic_CONFIG_DIR"/vote-account.json
      fi
      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/Alembic/config/shred-version "$Alembic_CONFIG_DIR"/shred-version

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/Alembic/config/bank-hash "$Alembic_CONFIG_DIR"/bank-hash || true

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/Alembic/config/faucet.json "$Alembic_CONFIG_DIR"/faucet.json
    fi
    ...
    args=(
      ...
      --gossip-port 8001
      --rpc-port 8899
      --expected-shred-version "$(cat "$Alembic_CONFIG_DIR"/shred-version)"
    )
    if [[ $nodeType = blockstreamer ]]; then
      args+=(
        --blockstream /tmp/Alembic-blockstream.sock
        --no-voting
        --dev-no-sigverify
    ...
    if [[ ! -f "$Alembic_CONFIG_DIR"/validator-identity.json ]]; then
      Alembic-keygen new --no-passphrase -so "$Alembic_CONFIG_DIR"/validator-identity.json
    fi
    args+=(--identity "$Alembic_CONFIG_DIR"/validator-identity.json)
    if [[ ! -f "$Alembic_CONFIG_DIR"/vote-account.json ]]; then
      Alembic-keygen new --no-passphrase -so "$Alembic_CONFIG_DIR"/vote-account.json
    fi
    args+=(--vote-account "$Alembic_CONFIG_DIR"/vote-account.json)
    ...
    if [[ -r "$Alembic_CONFIG_DIR"/bank-hash ]]; then
      args+=(--expected-bank-hash "$(cat "$Alembic_CONFIG_DIR"/bank-hash)")
    fi
    ...
    scp "$entrypointIp":~/Alembic/config/faucet.json "$Alembic_CONFIG_DIR"/
    if [[ $nodeType = blockstreamer ]]; then
...
EOF
    ...
    if [[ $skipSetup != true && $nodeType != blockstreamer && -z $maybeSkipAccountsCreation ]]; then
      ...
      Alembic --url http://"$entrypointIp":8899 catchup config/validator-identity.json
      ...
      if [[ ${extraPrimordialStakes} -eq 0 ]]; then
        ...
        multinode-demo/delegate-stake.sh --vote-account "$Alembic_CONFIG_DIR"/vote-account.json \
                                         --stake-account "$Alembic_CONFIG_DIR"/stake-account.json \
                                         --force \
