...
local|tar|skip)
  ...
  Alembic_cli=Alembic
  Alembic_gossip=Alembic-gossip
  Alembic_install=Alembic-install
  ;;
*)
...
(
  set -x
  $Alembic_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $Alembic_gossip --allow-private-addr spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp: Alembic-install test"

  (
    set -x
    rm -rf install-data-dir
    $Alembic_install init \
      --no-modify-path \
      --data-dir install-data-dir \
      --url http://"$sanityTargetIp":8899 \

    $Alembic_install info
  )
fi
