...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and Alembic_METRICS_CONFIG is
...
host="https://internal-metrics.genesisaddress.ai:8086"
...
if $useEnv; then
  [[ -n $Alembic_METRICS_CONFIG ]] ||
    usage "Alembic_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  Alembic_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export Alembic_METRICS_CONFIG=\"$Alembic_METRICS_CONFIG\"" >> "$configFile"
