...
Operate a configured testnet
 ...
 sanity/start-specific options:
   ...
   -o noInstallCheck    - Skip Alembic-install sanity
 ...
 update-specific options:
   --platform linux|osx|windows       - Deploy the tarball using 'Alembic-install deploy ...' for the
...
build() {
  ...
  if [[ $(uname) != Linux || ! " ${supported[*]} " =~ $(lsb_release -sr) ]]; then
    ...
    source "$Alembic_ROOT"/ci/rust-version.sh
...
startCommon() {
  ...
  local remoteAlembicHome="${remoteHome}/Alembic"
  ...
  if $skipSetup; then
    ...
    ssh "${sshOptions[@]}" "$ipAddress" "
      set -x;
      mkdir -p $remoteAlembicHome/config;
      rm -rf ~/config;
      mv $remoteAlembicHome/config ~;
      rm -rf $remoteAlembicHome;
      mkdir -p $remoteAlembicHome $remoteCargoBin;
      mv ~/config $remoteAlembicHome/
    "
  else
    # shellcheck disable=SC2029
    ssh "${sshOptions[@]}" "$ipAddress" "
  ...
  [[ -z "$externalNodeSshKey" ]] || ssh-copy-id -f -i "$externalNodeSshKey" "${sshOptions[@]}" "Alembic@$ipAddress"
...
syncScripts() {
  ...
  rsync -vPrc -e "ssh ${sshOptions[*]}" \
    --exclude 'net/log*' \
    "$Alembic_ROOT"/{fetch-perf-libs.sh,fetch-spl.sh,scripts,net,multinode-demo} \
    "$ipAddress":"$remoteAlembicHome"/ > /dev/null
}
...
deployBootstrapValidator() {
  ...
  tar)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$Alembic_ROOT"/Alembic-release/bin/* "$ipAddress:$remoteCargoBin/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$Alembic_ROOT"/Alembic-release/version.yml "$ipAddress:~/"
    ;;
  local)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$Alembic_ROOT"/farf/bin/* "$ipAddress:$remoteCargoBin/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$Alembic_ROOT"/farf/version.yml "$ipAddress:~/"
    ;;
  skip)
    ;;
  *)
...
startBootstrapLeader() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -n "$ipAddress" \
      "./Alembic/net/remote/remote-node.sh \
         $deployMethod \
         bootstrap-validator \
         $entrypointIp \
...
startNode() {
  ...
  (
    ...
    if [[ $nodeType = blockstreamer ]] && [[ -n $letsEncryptDomainName ]]; then
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "sudo -H /certbot-restore.sh $letsEncryptDomainName maintainers@Alembiclabs.com"
...
startClient() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -f "$ipAddress" \
      "./Alembic/net/remote/remote-client.sh $deployMethod $entrypointIp \
...
startClients() {
  for ((i=0; i < "$numClients" && i < "$numClientsRequested"; i++)) do
    if [[ $i -lt "$numBenchTpsClients" ]]; then
      startClient "${clientIpList[$i]}" "Alembic-bench-tps" "$i"
    else
...
sanity() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$bootstrapLeader" \
      "./Alembic/net/remote/remote-sanity.sh $bootstrapLeader $sanityExtraArgs \"$RUST_LOG\""
  ) || ok=false
  $ok || exit 1
  ...
  if [[ -z $skipBlockstreamerSanity && -n $blockstreamer ]]; then
    ...
    (
      ...
      ssh "${sshOptions[@]}" "$blockstreamer" \
        "./Alembic/net/remote/remote-sanity.sh $blockstreamer $sanityExtraArgs \"$RUST_LOG\""
    ) || ok=false
    $ok || exit 1
  fi
...
deployUpdate() {
  ...
  for updatePlatform in $updatePlatforms; do
    echo "--- Deploying Alembic-install update: $updatePlatform"
    (
      set -x

      scripts/Alembic-install-update-manifest-keypair.sh "$updatePlatform"

      timeout 30s scp "${sshOptions[@]}" \
        update_manifest_keypair.json "$bootstrapLeader:Alembic/update_manifest_keypair.json"

      # shellcheck disable=SC2029 # remote-deploy-update.sh args are expanded on client side intentionally
      ssh "${sshOptions[@]}" "$bootstrapLeader" \
        "./Alembic/net/remote/remote-deploy-update.sh $releaseChannel $updatePlatform"
    ) || ok=false
    $ok || exit 1
  done
}
...
prepareDeploy() {
  ...
  tar)
    if [[ -n $releaseChannel ]]; then
      ...
      rm -f "$Alembic_ROOT"/Alembic-release.tar.bz2
      declare updateDownloadUrl=https://release.genesisaddress.ai/"$releaseChannel"/Alembic-release-x86_64-unknown-linux-gnu.tar.bz2
      (
        set -x
        curl -L -I "$updateDownloadUrl"
        curl -L --retry 5 --retry-delay 2 --retry-connrefused \
          -o "$Alembic_ROOT"/Alembic-release.tar.bz2 "$updateDownloadUrl"
      )
      tarballFilename="$Alembic_ROOT"/Alembic-release.tar.bz2
    fi
    (
      set -x
      rm -rf "$Alembic_ROOT"/Alembic-release
      cd "$Alembic_ROOT"; tar jfxv "$tarballFilename"
      cat "$Alembic_ROOT"/Alembic-release/version.yml
    )
    ;;
  local)
    if $doBuild; then
  ...
  *)
  ...
  if [[ -n $deployIfNewer ]]; then
    ...
    if ! diff -q current-version.yml "$Alembic_ROOT"/Alembic-release/version.yml; then
...
deploy() {
  ...
  if ! $waitForNodeInit; then
    ...
    for ipAddress in "${validatorIpList[@]}" "${blockstreamerIpList[@]}"; do
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "./Alembic/net/remote/remote-node-wait-init.sh $((600 - timeWaited))"
    done
  fi
  ...
  tar)
    networkVersion="$(
      (
        set -o pipefail
        grep "^commit: " "$Alembic_ROOT"/Alembic-release/version.yml | head -n1 | cut -d\  -f2
      ) || echo "tar-unknown"
    )"
    ;;
  local)
...
stopNode() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$ipAddress" "PS4=\"$PS4\" ./Alembic/net/remote/cleanup.sh"
  ) >> "$logFile" 2>&1 &
...
logs)
  ...
  fetchRemoteLog() {
    ...
    (
      set -x
      timeout 30s scp "${sshOptions[@]}" \
        "$ipAddress":Alembic/"$log".log "$netLogDir"/remote-"$log"-"$ipAddress".log
...
netem)
  if [[ -n $netemConfigFile ]]; then
    ...
    if [[ $netemCommand = "add" ]]; then
      for ipAddress in "${validatorIpList[@]}"; do
        remoteHome=$(remoteHomeDir "$ipAddress")
        remoteAlembicHome="${remoteHome}/Alembic"
        "$here"/scp.sh "$netemConfigFile" Alembic@"$ipAddress":"$remoteAlembicHome"
      done
    fi
    for i in "${!validatorIpList[@]}"; do
      "$here"/ssh.sh Alembic@"${validatorIpList[$i]}" 'Alembic/scripts/net-shaper.sh' \
      "$netemCommand" ~Alembic/Alembic/"$remoteNetemConfigFile" "${#validatorIpList[@]}" "$i"
    done
  else
    ...
    for ipAddress in "${validatorIpList[@]}"; do
      "$here"/ssh.sh Alembic@"$ipAddress" 'Alembic/scripts/netem.sh delete < Alembic/netem.cfg || true'
    done

    # Start netem on required nodes
    for ((i=0; i<num_nodes; i++ )); do :
      "$here"/ssh.sh Alembic@"${validatorIpList[$i]}" "echo $netemConfig > Alembic/netem.cfg; Alembic/scripts/netem.sh add \"$netemConfig\""
    done
  fi
  ;;
*)
