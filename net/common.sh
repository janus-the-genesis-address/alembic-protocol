...
source "$Alembic_ROOT"/scripts/configure-metrics.sh
...
Alembic_CONFIG_DIR=$Alembic_ROOT/config
...
_setup_secondary_mount() {
  ...
  (
    ...
    if [[ -d $SECONDARY_DISK_MOUNT_POINT ]] && \
      ...
      rm -rf "$Alembic_CONFIG_DIR"
      ln -sfT $SECONDARY_DISK_MOUNT_POINT/config "$Alembic_CONFIG_DIR"
    fi
  )
}
