...
#[deprecated(
    since = "1.8.0",
    note = "Please use `Alembic_sdk::stake::state` or `Alembic_program::stake::state` instead"
)]
pub use Alembic_sdk::stake::state::*;
use {
    Alembic_program_runtime::{ic_msg, invoke_context::InvokeContext},
    Alembic_sdk::{
...
pub fn delegate(
    ...
    if *vote_account.get_owner() != Alembic_vote_program::id() {
...
pub(crate) fn deactivate_delinquent(
    ...
    if *delinquent_vote_account.get_owner() != Alembic_vote_program::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
    ...
    if *reference_vote_account.get_owner() != Alembic_vote_program::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
...
mod tests {
    use {
        super::*,
        proptest::prelude::*,
        Alembic_program_runtime::with_mock_invoke_context,
        Alembic_sdk::{
    ...
    fn test_authorized_authorize() {
        let staker = Alembic_sdk::pubkey::new_rand();
    ...
    fn test_authorized_authorize_with_custodian() {
        ...
        let invalid_custodian = Alembic_sdk::pubkey::new_rand();
        let mut authorized = Authorized::auto(&staker);
        let mut signers = HashSet::new();
        signers.insert(staker);
    ...
    fn test_dbg_stake_minimum_balance() {
        ...
        panic!(
            "stake minimum_balance: {} lamports, {} TACHYON",
            minimum_balance,
            minimum_balance as f64 / Alembic_sdk::native_token::LAMPORTS_PER_SOL as f64
        );
    }
