...
pub use Alembic_perf::report_target_features;
use {
    crate::{
    ...
    Alembic_entry::poh::compute_hash_time_ns,
    Alembic_geyser_plugin_manager::{
    ...
    Alembic_streamer::{socket::SocketAddrSpace, streamer::StakedNodes},
    Alembic_turbine::{self, broadcast_stage::BroadcastStageType},
    Alembic_unified_scheduler_pool::DefaultSchedulerPool,
    Alembic_vote_program::vote_state,
    Alembic_wen_restart::wen_restart::wait_for_wen_restart,
    std::{
...
pub struct Validator {
    ...
    tpu: Tpu,
    tvu: Tvu,
    ip_echo_server: Option<Alembic_net_utils::IpEchoServer>,
    ...
    turbine_quic_endpoint_join_handle: Option<Alembic_turbine::quic_endpoint::AsyncTryJoinHandle>,
...
impl Validator {
    ...
    pub fn new(
        ...
        if Alembic_perf::perf_libs::api().is_some() {
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        let ip_echo_server = match node.sockets.ip_echo {
            None => None,
            Some(tcp_listener) => Some(Alembic_net_utils::ip_echo_server(
                tcp_listener,
        ...
        let (
            ...
            Alembic_turbine::quic_endpoint::new_quic_endpoint(
        ...
        datapoint_info!(
            ...
            ("version", Alembic_version::version!(), String),
    ...
    pub fn join(self) {
        ...
        if let Some(turbine_quic_endpoint) = &self.turbine_quic_endpoint {
            Alembic_turbine::quic_endpoint::close_quic_endpoint(turbine_quic_endpoint);
        }
...
fn maybe_warp_slot(
    ...
    if let Some(warp_slot) = config.warp_slot {
        ...
        bank_forks.insert(Bank::warp_from_parent(
            ...
            &Pubkey::default(),
            warp_slot,
            Alembic_accounts_db::accounts_db::CalcAccountsHashDataSource::Storages,
        ));
        bank_forks.set_root(
            warp_slot,
            accounts_background_request_sender,
...
mod tests {
    use {
        ...
        Alembic_entry::entry,
        Alembic_gossip::contact_info::{ContactInfo, LegacyContactInfo},
        Alembic_ledger::{
            blockstore, create_new_tmp_ledger, genesis_utils::create_genesis_config_with_leader,
            get_tmp_ledger_path_auto_delete,
        },
        Alembic_sdk::{genesis_config::create_genesis_config, poh_config::PohConfig},
        Alembic_tpu_client::tpu_client::{
    ...
    fn test_poh_speed() {
        ...
        let poh_config = PohConfig {
            target_tick_duration: Duration::from_millis(Alembic_sdk::clock::MS_PER_TICK),
            // make PoH rate really fast to cause the panic condition
            hashes_per_tick: Some(100 * Alembic_sdk::clock::DEFAULT_HASHES_PER_TICK),
            ..PohConfig::default()
        };
