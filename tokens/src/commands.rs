use {
    ...
    Alembic_account_decoder::parse_token::real_number_string,
    ...
    spl_token::Alembic_program::program_error::ProgramError,
    std::{
        cmp::{self},
        io,
        str::FromStr,
...
fn read_allocations(
    ...
    let allocations = if let Some(amount) = transfer_amount {
        ...
        // We only support TACHYON token in "require lockup" mode.
        rdr.deserialize()
            .map(|recipient| {
...
pub fn process_allocations(
    ...
    let starting_total_tokens = if let Some(spl_token_args) = &args.spl_token_args {
        ...
        Token::TACHYON(starting_total_tokens)
    };
    println!(
        "{} {}",
    ...
    let (distributed_tokens, undistributed_tokens) =
        if let Some(spl_token_args) = &args.spl_token_args {
            ...
            (
                Token::TACHYON(distributed_tokens),
                Token::TACHYON(undistributed_tokens),
            )
        };
