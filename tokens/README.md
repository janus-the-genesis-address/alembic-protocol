...
```bash
Alembic-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
Alembic-tokens transaction-log --output-path transactions.csv
```
...
```bash
Alembic-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
Alembic-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
Alembic-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
    --input-csv <ALLOCATIONS_CSV> \
    --stake-authority <KEYPAIR> --withdraw-authority <KEYPAIR> --fee-payer <KEYPAIR>
```

Currently, this will subtract 1 TACHYON from each allocation and store it in the
recipient address. That TACHYON can be used to pay transaction fees on staking
...
Distributing SPL Tokens works very similarly to distributing TACHYON, but requires
...
```bash
Alembic-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
Alembic-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
As with TACHYON, you can List the differences between a list of expected
distributions and the record of what transactions have already been sent using
the `--dry-run` parameter, or `Alembic-tokens balances`.
...
```bash
Alembic-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
