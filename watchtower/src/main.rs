...
use {
    ...
    Alembic_cli_output::display::format_labeled_address,
    Alembic_metrics::{datapoint_error, datapoint_info},
    Alembic_notifier::{NotificationType, Notifier},
    Alembic_rpc_client::rpc_client::RpcClient,
    Alembic_rpc_client_api::{client_error, response::RpcVoteAccountStatus},
    Alembic_sdk::{
        hash::Hash,
        native_token::{sol_to_lamports, TACHYON},
        pubkey::Pubkey,
    },
    std::{
        collections::HashMap,
...
fn get_config() -> Config {
    let matches = App::new(crate_name!())
        ...
        define environment variables before running `Alembic-watchtower`:
        ...
        define environment variable before running `Alembic-watchtower`:
        ...
        .arg(
            Arg::with_name("minimum_validator_identity_balance")
                ...
                .help("Alert when the validator identity balance is less than this amount of TACHYON")
        )
        .arg(
        ...
        .arg(
            Arg::with_name("name_suffix")
                ...
                .takes_value(true)
                .default_value("")
                .help("Add this string into all notification messages after \"Alembic-watchtower\"")
        )
        .get_matches();

    let config = if let Some(config_file) = matches.value_of("config_file") {
        Alembic_cli_config::Config::load(config_file).unwrap_or_default()
    } else {
        Alembic_cli_config::Config::default()
    };
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    Alembic_metrics::set_panic_hook("watchtower", /*version:*/ None);
    ...
    loop {
        let failure = match get_cluster_info(&config, &rpc_client) {
            Ok((transaction_count, recent_blockhash, vote_accounts, validator_balances)) => {
                ...
                info!(
                    ...
                    TACHYON(total_stake),
                    TACHYON(total_current_stake),
                    TACHYON(total_delinquent_stake)
                );
                ...
                for validator_identity in config.validator_identity_pubkeys.iter() {
                    ...
                    if let Some(balance) = validator_balances.get(validator_identity) {
                        if *balance < config.minimum_validator_identity_balance {
                            failures.push((
                                "balance",
                                format!("{} has {}", formatted_validator_identity, TACHYON(*balance)),
                            ));
                        }
                    }
                }
        ...
        if let Some((failure_test_name, failure_error_message)) = &failure {
            ...
            if !last_notification_msg.is_empty() {
                ...
                info!("{}", all_clear_msg);
                notifier.send(
                    &format!("Alembic-watchtower{}: {}", config.name_suffix, all_clear_msg),
