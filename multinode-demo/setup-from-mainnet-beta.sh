...
rm -rf "$Alembic_CONFIG_DIR"/latest-mainnet-beta-snapshot
mkdir -p "$Alembic_CONFIG_DIR"/latest-mainnet-beta-snapshot
(
  cd "$Alembic_CONFIG_DIR"/latest-mainnet-beta-snapshot || exit 1
  set -x
  wget http://api.mainnet-beta.genesisaddress.ai/genesis.tar.bz2
  wget --trust-server-names http://api.mainnet-beta.genesisaddress.ai/snapshot.tar.bz2
)

snapshot=$(ls "$Alembic_CONFIG_DIR"/latest-mainnet-beta-snapshot/snapshot-[0-9]*-*.tar.zst)
if [[ -z $snapshot ]]; then
...
$Alembic_ledger_tool create-snapshot \
  --ledger "$Alembic_CONFIG_DIR"/latest-mainnet-beta-snapshot \
