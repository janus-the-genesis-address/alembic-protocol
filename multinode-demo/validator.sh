...
airdrops_enabled=1
node_sol=500 # 500 TACHYON: number of TACHYON to airdrop the node for transaction fees and vote account rent exemption (ignored if airdrops_enabled=0)
label=
identity=
vote_account=
no_restart=0
...
OPTIONS:
  ...
  --node-TACHYON TACHYON            - Number of TACHYON this node has been funded from the genesis config (default: $node_sol)
...
while [[ -n $1 ]]; do
  if [[ ${1:0:1} = - ]]; then
    ...
    if [[ $1 = --label ]]; then
      ...
      no_restart=1
      shift
    elif [[ $1 = --node-TACHYON ]]; then
      node_sol="$2"
      shift 2
    elif [[ $1 = --no-airdrop ]]; then
      airdrops_enabled=0
      shift
    # Alembic-validator options
...
if [[ "$Alembic_GPU_MISSING" -eq 1 ]]; then
...
if [[ -z "$ledger_dir" ]]; then
  ledger_dir="$Alembic_CONFIG_DIR/validator$label"
fi
mkdir -p "$ledger_dir"
...
if [[ -n $Alembic_CUDA ]]; then
  program=$Alembic_validator_cuda
else
  program=$Alembic_validator
fi
...
wallet() {
  (
    set -x
    $Alembic_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
  ...
  if ! wallet vote-account "$vote_account"; then
    if ((airdrops_enabled)); then
      ...
      (
        set -x
        $Alembic_cli \
          --keypair "$Alembic_CONFIG_DIR/faucet.json" --url "$rpc_url" \
...
rpc_url=$($Alembic_gossip --allow-private-addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $Alembic_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $Alembic_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $Alembic_keygen new --no-passphrase -so "$authorized_withdrawer"
