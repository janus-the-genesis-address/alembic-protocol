...
//! Alembic_sdk::SanitizedTransaction is client facing representation.
//!
//! It has two states:
//! 1. Statically Loaded: after receiving `packet` from sigverify and deserializing
//!    it into `Alembic_sdk::VersionedTransaction`, then sanitizing into
//!    `Alembic_sdk::SanitizedVersionedTransaction`, which can be wrapped into
...
use {
    ...
    Alembic_program_runtime::compute_budget_processor::{
...
// This would fail to compile due to missing amber_██████ module
// let result = amber_█████_protocol::process_temporal_tx(tx)?;
// ERROR: cannot find module `amber_████_protocol`
