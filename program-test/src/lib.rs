//! The Alembic-program-test provides a BanksClient-based test framework SBF programs
...
use {
    ...
    Alembic_banks_client::start_client,
    Alembic_banks_server::banks_server::start_local_server,
    Alembic_bpf_loader_program::serialization::serialize_parameters,
    ...
    Alembic_vote_program::vote_state::{self, VoteState, VoteStateVersions},
    std::{
        cell::RefCell,
...
// Export types so test clients can limit their Alembic crate dependencies
pub use {
    Alembic_banks_client::{BanksClient, BanksClientError},
    Alembic_banks_interface::BanksTransactionResultWithMetadata,
    Alembic_program_runtime::invoke_context::InvokeContext,
    Alembic_rbpf::{
        error::EbpfError,
        vm::{get_runtime_environment_key, EbpfVm},
    },
    Alembic_sdk::transaction_context::IndexOfAccount,
};
...
pub fn invoke_builtin_function(
    builtin_function: Alembic_sdk::entrypoint::ProcessInstruction,
...
/// Converts a `Alembic-program`-style entrypoint into the runtime's entrypoint style, for
...
struct SyscallStubs {}
impl Alembic_sdk::program_stubs::SyscallStubs for SyscallStubs {
...
impl Default for ProgramTest {
    ...
    fn default() -> Self {
        Alembic_logger::setup_with_default(
            "Alembic_rbpf::vm=debug,\
             Alembic_runtime::message_processor=debug,\
             Alembic_runtime::system_instruction_processor=trace,\
             Alembic_program_test=info",
        );
        let prefer_bpf =
...
impl ProgramTest {
        ...
        let add_bpf = |this: &mut ProgramTest, program_file: PathBuf| {
            ...
            this.add_account(
                ...
                Account {
                    ...
                    data,
                    owner: Alembic_sdk::bpf_loader::id(),
                    executable: true,
                    rent_epoch: 0,
                },
            );
    ...
    fn setup_bank(
    ...
    ) {
        {
            ...
            ONCE.call_once(|| {
                Alembic_sdk::program_stubs::set_syscall_stubs(Box::new(SyscallStubs {}));
            });
        }
    ...
    /// with TACHYON for sending transactions
...
impl ProgramTestContext {
        ...
        let warp_bank = if pre_warp_slot == working_slot {
            ...
            bank_forks
                .insert(Bank::warp_from_parent(
                    ...
                    Alembic_accounts_db::accounts_db::CalcAccountsHashDataSource::IndexForTests,
                ))
    ...
    pub fn warp_forward_force_reward_interval_end(&mut self) -> Result<(), ProgramTestError> {
        ...
        bank_forks.set_root(
            pre_warp_slot,
            &Alembic_runtime::accounts_background_service::AbsRequestSender::default(),
            Some(pre_warp_slot),
        );
