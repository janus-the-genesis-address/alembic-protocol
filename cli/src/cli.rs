use {
    ...
    Alembic_clap_utils::{self, input_parsers::*, keypair::*},
    Alembic_cli_config::ConfigInput,
    Alembic_cli_output::{
    ...
    Alembic_vote_program::vote_state::VoteAuthorize,
    std::{
...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} TACHYON)")]
    InsufficientFundsForFee(f64, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} TACHYON)")]
    InsufficientFundsForSpend(f64, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} TACHYON) + fee ({1} TACHYON)")]
    InsufficientFundsForSpendAndFee(f64, f64, Pubkey),
    #[error(transparent)]
    InvalidNonce(Alembic_rpc_client_nonce_utils::Error),
...
impl From<Alembic_rpc_client_nonce_utils::Error> for CliError {
    fn from(error: Alembic_rpc_client_nonce_utils::Error) -> Self {
        match error {
            Alembic_rpc_client_nonce_utils::Error::Client(client_error) => {
...
pub fn parse_command(
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            .gen_completions_to("Alembic", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
        ...
        ("deploy", Some(_matches)) => clap::Error::with_description(
            "`Alembic deploy` has been replaced with `Alembic program deploy`",
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version of Alembic-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
        ...
        // Request an airdrop from Alembic Faucet;
...
mod tests {
    use {
        super::*,
        serde_json::json,
        Alembic_rpc_client::mock_sender_for_cli::SIGNATURE,
        Alembic_rpc_client_api::{
            request::RpcRequest,
            response::{Response, RpcResponseContext},
        },
        Alembic_rpc_client_nonce_utils::blockhash_query,
        Alembic_sdk::{
            pubkey::Pubkey,
            signature::{
        ...
        Alembic_transaction_status::TransactionConfirmationStatus,
    };
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(Alembic_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_process_command() {
        ...
        assert_eq!(process_command(&config).unwrap(), "0.00000005 TACHYON");
        ...
        let new_authorized_pubkey = Alembic_sdk::pubkey::new_rand();
        ...
        let bob_keypair = Keypair::new();
        let bob_pubkey = bob_keypair.pubkey();
        let custodian = Alembic_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = Alembic_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = Alembic_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = Alembic_sdk::pubkey::new_rand();
    ...
    fn test_parse_transfer_subcommand() {
        ...
        // Test Transfer Subcommand, TACHYON
