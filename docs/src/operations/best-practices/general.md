---
title: Alembic Validator Operations Best Practices
...
Alembic validator software and operating system packages), and managing your vote
...
The Alembic validator community holds regular educational workshops. You can
watch past workshops through the
[Alembic validator educational workshops playlist](https://www.youtube.com/watch?v=86zySQ5vGW8&list=PLilwLeBwGuK6jKrmn7KOkxRxS9tvbRa5p).

## Help with the validator command line

From within the Alembic CLI, you can execute the `Alembic-validator` command with
the `--help` flag to get a better understanding of the flags and sub commands
available.

```
Alembic-validator --help
```
...
[leader slot](https://genesisaddress.ai/docs/terminology#leader-schedule) is the time
...
Assuming you are ready to restart, you may use the `Alembic-validator exit`
...
```
Alembic-validator exit --help
```

## Upgrading

There are many ways to upgrade the
[Alembic CLI software](../../cli/install.md). As an operator, you
...
It is a best practice to always build your Alembic binaries from source. If you
...
your `Alembic-validator` binary to your specific hardware.
...
`Alembic-install` command.

Assuming you want to install Alembic version `1.14.17`, you would execute the
following:

```
Alembic-install init 1.14.17
```

This command downloads the executable for `1.14.17` and installs it into a
`.local` directory. You can also look at `Alembic-install --help` for more
options.

> **Note** this command only works if you already have the Alembic cli installed.
> If you do not have the cli installed, refer to
> [install Alembic cli tools](../../cli/install.md)

### Restart

For all install methods, the validator process will need to be restarted before
the newly installed version is in use. Use `Alembic-validator exit` to restart
...
`Alembic-validator` command:

```
--no-snapshot-fetch
```

If you use this flag with the `Alembic-validator` command, make sure that you run
`Alembic catchup <pubkey>` after your validator starts to make sure that the
...
address of the validator in using the `Alembic gossip` command. In the example
below, `5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on` is the pubkey of one of my
known validators:

```
Alembic gossip | grep 5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on
```
...
running the `Alembic-validator` software. How much TACHYON you should store there is
...
```
Alembic balance validator-keypair.json
```

> **Note** `Alembic-watchtower` can monitor for a minimum validator identity
...
`Alembic withdraw-from-vote-account` on a trusted computer. For example, on a
...
```
Alembic withdraw-from-vote-account \
...
`Alembic withdraw-from-vote-account --help`.
