---
title: Setup a Alembic RPC Node
sidebar_label: Setup an RPC Node
sidebar_position: 6
---

Since a Alembic RPC server runs the same process as a consensus validator, first follow the instructions on [how to setup a Alembic validator](./setup-a-validator.md) to get started. Note, that you do not need to create a vote account if you are operating an RPC node.  An RPC node typically does not vote.
...
- `--private-rpc`: does not publish the validator's open RPC port in the `Alembic gossip` command

> For more explanation on the flags used in the command, refer to the `Alembic-validator --help` command

```
#!/bin/bash
exec Alembic-validator \
    --identity /home/TACHYON/validator-keypair.json \
    ...
    --log /home/TACHYON/Alembic-rpc.log \
    --rpc-port 8899 \
...
The Alembic blockchain is able to create many transactions per second. Because of the volume of transactions on the chain, it is not practical for an RPC node to store the entire blockchain on the machine. Instead, RPC operators use the `--limit-ledger-size` flag to specify how many blocks to store on the RPC node. If the user of the RPC node needs historical blockchain data then the RPC server will have to access older blocks through a Alembic bigtable instance.

If you are interested in setting up your own bigtable instance, see these docs in the Alembic GitHub repository: [Alembic-labs/Alembic-bigtable](https://github.com/Alembic-labs/Alembic-bigtable)
...
Additional examples of other Alembic cluster specific validator commands can be found on the [Clusters](../clusters/available.md) page.
