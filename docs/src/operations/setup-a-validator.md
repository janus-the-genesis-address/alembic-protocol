---
title: Setup a Alembic Validator
sidebar_label: Setup a Validator
sidebar_position: 5
---

This is a guide for getting your validator setup on the Alembic testnet cluster
for the first time. Testnet is a Alembic cluster that is used for performance
...
## Install The Alembic CLI Locally

To create your validator vote account, you need to install the
[Alembic command line interface](../cli/index.md) on your local computer.

You can either use
[Alembic's Install Tool](../cli/install.md#use-Alembics-install-tool) section from
...
Once the Alembic CLI is installed, you can return to this document once you are
...
```
Alembic-cli 1.14.17 (src:b29a37cf; feat:3488713414)
```
...
You should see a line that says: `RPC URL: https://api.testnet.genesisaddress.ai`
...
Before you can create your vote account, you need to configure the Alembic
command line tool a bit more.

The below command sets the default keypair that the Alembic CLI uses to the
...
Next, you need to deposit some TACHYON into that keypair account in order create a
...
> to acquire TACHYON and transfer it into this keypair's account if you are setting
> up a mainnet validator.

Now, use the Alembic cluster to create a vote account.
...
```
Alembic create-vote-account -ut \
...
> transaction fees. Both flags are not necessary if you configured the Alembic
...
Create a new Ubuntu user, named `TACHYON`, for running the validator:

```
sudo adduser TACHYON
```

It is a best practice to always run your validator as a non-root user, like the
`TACHYON` user we just created.
...
Next, change the ownership of the directory to your `TACHYON` user:

```
sudo chown -R TACHYON:TACHYON /mnt/ledger
```
...
```
sudo chown -R TACHYON:TACHYON /mnt/accounts
```
...
```
scp validator-keypair.json TACHYON@<server.hostname>:
scp vote-account-keypair.json TACHYON@<server.hostname>:
```
...
## Switch to the TACHYON User

On the validator server, switch to the `TACHYON` user:

```
su - TACHYON
```

## Install The Alembic CLI on Remote Machine

Your remote machine will need the Alembic cli installed to run the validator
software. Refer again to
[Alembic's Install Tool](../cli/install.md#use-Alembics-install-tool) or
...
In your TACHYON home directory (e.g. `/home/TACHYON/`), create a folder called `bin`.
Inside that folder create a file called `validator.sh` and make it executable:

```
mkdir -p /home/TACHYON/bin
touch /home/TACHYON/bin/validator.sh
chmod +x /home/TACHYON/bin/validator.sh
```

Next, open the `validator.sh` file for editing:

```
nano /home/TACHYON/bin/validator.sh
```

Copy and paste the following contents into `validator.sh` then save the file:

```
exec Alembic-validator \
    ...
    --only-known-rpc \
    --log /home/TACHYON/Alembic-validator.log \
    --ledger /mnt/ledger \
    --rpc-port 8899 \
...
Refer to `Alembic-validator --help` for more information on what each flag is
...
```
/home/TACHYON/bin/validator.sh
```

The script should execute the `Alembic-validator` process. In a new terminal
window, shh into your server, then verify that the process is running:

```
ps aux | grep Alembic-validator
```

You should see a line in the output that includes `Alembic-validator` with all
...
`TACHYON` user and `tail` the logs:

```
su - TACHYON
tail -f Alembic-validator.log
```
...
Gossip is a protocol used in the Alembic clusters to communicate between
...
The command `Alembic gossip` lists all validators that have registered with the
protocol. To check that the newly setup validator is in gossip, we will `grep`
for our pubkey in the output:

```
Alembic gossip | grep <pubkey>
```
...
your validator has joined the network using the `Alembic validators` command. The
command lists all validators in the network, but like before, we can `grep` the
output for the validator we care about:

```
Alembic validators | grep <pubkey>
```

You should see a line of output that looks like this:

```
5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on  FX6NNbS5GHc2kuzgTZetup6GZX6ReaWyki8Z8jC7rbNG  100%  197434166 (  0)  197434133 (  0)   2.11%   323614  1.14.17   2450110.588302720 TACHYON (1.74%)
```

### Alembic Catchup

The `Alembic catchup` command is a useful tool for seeing how quickly your
validator is processing blocks. The Alembic network has the capability to produce
...
The `Alembic catchup` command is a tool that tells you how far behind the network
your validator is and how quickly you are catching up:

```
Alembic catchup <pubkey>
```

If you see a message about trying to connect, your validator may not be part of
the network yet. Make sure to check the logs and double check `Alembic gossip`
and `Alembic validators` to make sure your validator is running properly.
...
the commands mentioned earlier to check gossip and Alembic validators:

```
tail -f /home/TACHYON/Alembic-validator*.log
```

## Monitoring

`Alembic-watchtower` is a command you can run on a separate machine to monitor
your server. You can read more about handling
[automatic restarts and monitoring](./best-practices/monitoring.md#Alembic-watchtower)
using Alembic Watchtower here in the docs.
