...
### Primary Validator
The following additional `Alembic-validator` parameters are required to enable
...
following `Alembic-validator` command-line argument changes:
...
```bash
$ Alembic-validator wait-for-restart-window --identity validator-keypair.json \
  && Alembic-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `Alembic-validator set-identity
