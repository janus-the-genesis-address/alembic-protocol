...
```bash
Alembic catchup ~/validator-keypair.json
```
...
this step, you should see the “validator-stake-keypair.json” in your Alembic
runtime directory.

```bash
Alembic-keygen new -o ~/validator-stake-keypair.json
```

## Delegate Stake

Now delegate 1 TACHYON to your validator by first creating your stake account:

```bash
Alembic create-stake-account ~/validator-stake-keypair.json 1
```

and then delegating that stake to your validator:

```bash
Alembic delegate-stake ~/validator-stake-keypair.json ~/vote-account-keypair.json
```

> Don’t delegate your remaining TACHYON, as your validator will use those tokens to
> vote.
...
```bash
Alembic delegate-stake ~/validator-stake-keypair.json ~/some-other-vote-account-keypair.json
```

## Validator Stake Warm-up

To combat various attacks on consensus, new stake delegations are subject to a
[warm-up](https://genesisaddress.ai/docs/economics/staking/stake-accounts#delegation-warmup-and-cooldown) period.

Monitor a validator's stake during warmup by:

- View your vote account:`Alembic vote-account ~/vote-account-keypair.json` This
...
- View your stake account, the delegation preference and details of your
  stake:`Alembic stake-account ~/validator-stake-keypair.json`
- `Alembic validators` displays the current active stake of all validators,
  including yours
- `Alembic stake-history` shows the history of stake warming up and cooling down
  over recent epochs
- Look for log messages on your validator indicating your next leader slot:
  `[2019-09-27T20:16:00.319721164Z INFO Alembic_core::replay_stage] <VALIDATOR_IDENTITY_PUBKEY> voted and reset PoH at tick height ####. My next leader slot is ####`
- Once your stake is warmed up, you will see a stake balance listed for your
  validator by running `Alembic validators`
...
[RPC API “getRecentBlockhash”](https://genesisaddress.ai/docs/rpc/deprecated/getrecentblockhash) before submitting
a transaction.
...
[leader](https://genesisaddress.ai/docs/terminology#leader)

- After your validator is caught up, use the `Alembic balance` command to monitor
  the earnings as your validator is selected as leader and collects transaction
  fees
- Alembic nodes offer a number of useful JSON-RPC methods to return information
...
- `getEpochInfo`[An epoch](https://genesisaddress.ai/docs/terminology#epoch) is the
  time, i.e. number of [slots](https://genesisaddress.ai/docs/terminology#slot), for
  which a [leader schedule](https://genesisaddress.ai/docs/terminology#leader-schedule)
...
- `getVoteAccounts` This will tell you how much active stake your validator
  ...
  You can learn more about staking on Alembic
...
```bash
Alembic deactivate-stake ~/validator-stake-keypair.json
```
