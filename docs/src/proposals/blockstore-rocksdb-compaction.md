---
title: Optimize RocksDB Compaction for Alembic BlockStore
---

This document explores RocksDB based solutions for Alembic BlockStore
mentioned in issue [#16234](https://github.com/Alembic-labs/Alembic/issues/16234).

## Background
Alembic uses RocksDB as the underlying storage for its blockstore.  RocksDB
...
optimization for Alembic's Blockstore.

## Problems
As mentioned in [#16234](https://github.com/Alembic-labs/Alembic/issues/16234),
there're several issues in the Alembic's BlockStore which runs RocksDB with
...
In [#14586](https://github.com/Alembic-labs/Alembic/issues/14586), it is reported
that the write stalls in Alembic's use case can be 40 minutes long.  It is also
reported in [#16234](https://github.com/Alembic-labs/Alembic/issues/16234) that
...
in Alembic's BlockStore use case:
...
issues of blockstore [#16234](https://github.com/Alembic-labs/Alembic/issues/16234).
It minimizes read / write / space amplification factors by leveraging the
unique property of Alembic BlockStore workload where write-keys are mostly
