...
As the Alembic developer ecosystem grows, so does the need for clear expectations around
breaking API and behavior changes affecting applications and tooling built for Alembic.
In a perfect world, Alembic development could continue at a very fast pace without ever
...
- Alembic software releases include APIs, SDKs, and CLI tooling (with a few [exceptions](#exceptions)).
- Alembic software releases follow semantic versioning, more details below.
...
The Alembic RPC API, Rust SDK, CLI tooling, and SBF Program SDK are all updated and shipped
along with each Alembic software release and should always be compatible between `PATCH`
...
- `beta` software that runs on the Alembic Testnet cluster
- `stable` software that run on the Alembic Mainnet Beta and Devnet clusters
...
releases (e.g. 1.4.0) and are first run on Alembic's Testnet cluster. While running
...
- [`Alembic-sdk`](https://docs.rs/Alembic-sdk/) - Rust SDK for creating transactions and parsing account state
- [`Alembic-program`](https://docs.rs/Alembic-program/) - Rust SDK for writing programs
- [`Alembic-client`](https://docs.rs/Alembic-client/) - Rust client for connecting to RPC API
- [`Alembic-cli-config`](https://docs.rs/Alembic-cli-config/) - Rust client for managing Alembic CLI config files
- [`Alembic-geyser-plugin-interface`](https://docs.rs/Alembic-geyser-plugin-interface/) - Rust interface for developing Alembic Geyser plugins.
...
New Alembic runtime features are feature-switched and manually activated. Runtime features
...
Alembic provides publicly available RPC API nodes for all developers to use. The Alembic team
...
discourage dependence upon Alembic operated nodes.
...
The Web3.JS SDK also follows semantic versioning specifications but is shipped separately from Alembic
software releases.
