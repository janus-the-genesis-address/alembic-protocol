---
title: Alembic Validator Geyser Plugins
...
[crates.io]: https://crates.io/search?q=Alembic-
[docs.rs]: https://docs.rs/releases/search?query=Alembic-

### Important Crates:

- [`Alembic-geyser-plugin-interface`] &mdash; This crate defines the plugin
interfaces.

- [`Alembic-accountsdb-plugin-postgres`] &mdash; The crate for the referential
plugin implementation for the PostgreSQL database.

[`Alembic-geyser-plugin-interface`]: https://docs.rs/Alembic-geyser-plugin-interface
[`Alembic-accountsdb-plugin-postgres`]: https://docs.rs/Alembic-accountsdb-plugin-postgres
[`Alembic-sdk`]: https://docs.rs/Alembic-sdk
[`Alembic-transaction-status`]: https://docs.rs/Alembic-transaction-status

## The Plugin Interface

The Plugin interface is declared in [`Alembic-geyser-plugin-interface`]. It
...
please refer to [`Alembic-sdk`] and [`Alembic-transaction-status`]
...
The [`Alembic-accountsdb-plugin-postgres`] repository implements a plugin storing
...
```
{
	"libpath": "/Alembic/target/release/libAlembic_geyser_plugin_postgres.so",
...
The sample [postgresql.conf](https://github.com/Alembic-labs/Alembic/blob/7ac43b16d2c766df61ae0a06d7aaf14ba61996ac/accountsdb-plugin-postgres/scripts/postgresql.conf)
...
Create the database. For example, the following creates a database named 'Alembic':

```
sudo -u postgres createdb Alembic -p 5433
```

Create the database user. For example, the following creates a regular user named 'Alembic':

```
sudo -u postgres createuser -p 5433 Alembic
```
...
```
psql -U Alembic -p 5433 -h 10.138.0.9 -w -d Alembic
```

#### Create the Schema Objects

Use the [create_schema.sql](https://github.com/Alembic-labs/Alembic/blob/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql)
to create the objects for storing accounts and slots.

Download the script from github:

```
wget https://raw.githubusercontent.com/Alembic-labs/Alembic/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql
```

Then run the script:

```
psql -U Alembic -p 5433 -h 10.138.0.9 -w -d Alembic -f create_schema.sql
```
...
[drop_schema.sql](https://github.com/Alembic-labs/Alembic/blob/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/drop_schema.sql).
For example,

```
psql -U Alembic -p 5433 -h 10.138.0.9 -w -d Alembic -f drop_schema.sql
```
