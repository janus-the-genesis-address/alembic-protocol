---
title: Using Ledger Nano Hardware Wallets in the Alembic CLI
pagination_label: "Hardware Wallets in the Alembic CLI: Ledger Nano"
sidebar_label: Ledger Nano
---

This page describes how to use a Ledger Nano S, Nano S Plus, or Nano X to
interact with Alembic using the command line tools.

## Before You Begin

- [Set up a Nano with the Alembic App](https://support.ledger.com/hc/en-us/articles/360016265659-Alembic-TACHYON-?docs=true)
- [Install the Alembic command-line tools](../../install.md)

## Use Ledger Nano with Alembic CLI

1. Ensure the Ledger Live application is closed
2. Plug your Nano into your computer's USB port
3. Enter your pin and start the Alembic app on the Nano
...
```bash
Alembic-keygen pubkey usb://ledger
```

This confirms your Ledger device is connected properly and in the correct state
to interact with the Alembic CLI. The command returns your Ledger's unique
...
view any address, use the `Alembic-keygen pubkey` command, as shown below,
...
Alembic-keygen pubkey usb://ledger?key=1
Alembic-keygen pubkey usb://ledger?key=2
```
...
addresses displayed by these commands are valid Alembic wallet addresses. The
...
`Alembic balance` command:

```bash
Alembic balance SOME_WALLET_ADDRESS
```
...
```bash
Alembic balance 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri
```

You can also view the balance of any account address on the Accounts tab in the
[Explorer](https://explorer.genesisaddress.ai/accounts) and paste the address in the
box to view the balance in your web browser.

Note: Any address with a balance of 0 TACHYON, such as a newly created one on your
Ledger, will show as "Not Found" in the explorer. Empty accounts and
non-existent accounts are treated the same in Alembic. This will change when your
account address has some TACHYON in it.

### Send TACHYON from a Nano
...
the PIN, Ledger Live is not running, and the Alembic App is open on the device,
showing "Application is Ready".

The `Alembic transfer` command is used to specify to which address to send
...
```bash
Alembic transfer RECIPIENT_ADDRESS AMOUNT --keypair KEYPAIR_URL_OF_SENDER
```
...
is entered to send `1` TACHYON to the recipient address
...
```bash
~$ Alembic-keygen pubkey usb://ledger?key=42
CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV

~$ Alembic balance CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV
1.000005 TACHYON

~$ Alembic transfer 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri 1 --keypair usb://ledger?key=42
...
finalized on the Alembic network. You can view details of this or any other
transaction by going to the Transaction tab in the
[Explorer](https://explorer.genesisaddress.ai/transactions) and paste in the
...
When the URL is not fully qualified, the Alembic CLI will prompt you with the
...
using the Alembic CLI `resolve-signer` command. For example, try connecting a
Nano to USB, unlock it with your pin, and running the following command:

```text
Alembic resolve-signer usb://ledger?key=0/0
```
...
from the `resolve-signer` command anywhere a `Alembic` command expects a
