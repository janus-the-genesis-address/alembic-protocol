---
title: Using Hardware Wallets in the Alembic CLI
pagination_label: "Using Hardware Wallets in the Alembic CLI"
sidebar_label: Using in the Alembic CLI
sidebar_position: 0
---
...
The Alembic CLI has first class support for hardware wallets. Anywhere
...
The Alembic CLI supports the following hardware wallets:

- [Ledger Nano S and Ledger Nano X](./ledger.md)

## Specify a Keypair URL

Alembic defines a keypair URL format to uniquely locate any Alembic keypair on a
...
`DERVIATION_PATH` is used to navigate to Alembic keys within your hardware wallet.
...
and that any derived keys are Alembic keys (Coin type 501). The single quote
indicates a "hardened" derivation. Because Alembic uses Ed25519 keypairs, all
