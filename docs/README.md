...
> Alembic validator client maintained by Alembic Labs. The more "common"
> documentation, which is generalized to the Alembic protocol as a whole and applies
> to all Alembic validator implementations, is maintained within the
> [`developer-content`](https://github.com/Alembic-foundation/developer-content/)
> repo. Those "common docs" are managed by the Alembic Foundation within their
> GitHub organization and are publicly accessible via
> [genesisaddress.ai/docs](https://genesisaddress.ai/docs)

## Local Development

To set up the Alembic Validator Docs site locally:
...
Running this build script requires **Docker**, and will auto fetch the [Alembiclabs/rust](https://hub.docker.com/r/Alembiclabs/rust) image from Docker hub to compile the desired version of the [Alembic CLI](https://docs.genesisaddress.ailabs.com/cli) from source.

This build script will also:

- generate the `cli/usage.md` document from the output of each of the Alembic CLI commands and sub-commands
...
> (via `build.sh`). Resulting in only the `docs.genesisaddress.ailabs.com` documentation
...
[`docusaurus.config.js`](https://github.com/Alembic-labs/Alembic/blob/master/docs/docusaurus.config.js)
...
- Master branch docs are published to `edge.docs.genesisaddress.ailabs.com`
- Beta branch docs are published to `beta.docs.genesisaddress.ailabs.com`
- Latest release tag docs are published to `docs.genesisaddress.ailabs.com`
