...
use {
    ...
    Alembic_streamer::streamer::StakedNodes,
    std::{
        collections::HashMap,
        fs::File,
        io::prelude::*,
...
fn main() {
    ...
    Alembic_metrics::set_panic_hook("bench-tps", /*version:*/ None);

    let matches = cli::build_args(Alembic_version::version!()).get_matches();
    ...
    if let Some(instruction_padding_config) = instruction_padding_config {
        ...
        client
            ...
            .expect("Instruction padding program must be deployed to this cluster. Deploy the program using `Alembic program deploy ./bench-tps/tests/fixtures/spl_instruction_padding.so` and pass the resulting program id with `--instruction-padding-program-id`");
    }
