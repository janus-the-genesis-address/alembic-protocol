...
use {
    ...
    rand::{thread_rng, Rng},
    rayon::prelude::*,
    Alembic_clap_utils::input_parsers::pubkey_of,
    Alembic_cli::{
        cli::{process_command, CliCommand, CliConfig},
        program::ProgramCliCommand,
    },
    Alembic_client::transaction_executor::TransactionExecutor,
    Alembic_faucet::faucet::{request_airdrop_transaction, FAUCET_PORT},
...
fn main() {
    ...
    if let Some(addr) = matches.value_of("entrypoint") {
        entrypoint_addr = Alembic_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
    ...
    if let Some(addr) = matches.value_of("faucet_addr") {
        faucet_addr = Alembic_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
            eprintln!("failed to parse entrypoint address: {e}");
            exit(1)
        });
    }
...
pub mod test {
    use {
        ...
        Alembic_sdk::poh_config::PohConfig,
    };
    ...
    fn test_tx_size() {
        ...
        let signers: Vec<&Keypair> = vec![&keypair];
        let blockhash = Alembic_sdk::hash::Hash::default();
        let tx = Transaction::new(&signers, message, blockhash);
