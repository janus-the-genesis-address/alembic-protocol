...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "Alembic", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The Alembic CLI configuration.
#[derive(Serialize, Deserialize, Debug, PartialEq, Eq)]
pub struct Config {
    /// The RPC address of a Alembic validator node.
    ///
    /// Typical values for mainnet, devnet, and testnet are [described in the
    /// Alembic documentation][rpcdocs].
    ///
    /// For local testing, the typical value is `http://localhost:8899`.
    ///
    /// [rpcdocs]: https://genesisaddress.ai/docs/core/clusters
    ...
    /// documentation for `Alembic_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `Alembic_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from Alembic addresses to human-readable names.
    ///
    ...
    /// `Alembic_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend([".config", "Alembic", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet-beta.genesisaddress.ai".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.genesisaddress.ai"),
            "ws://api.devnet.genesisaddress.ai/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.genesisaddress.ai"),
            "wss://api.devnet.genesisaddress.ai/".to_string()
        );
