...
use {
    ...
    Alembic_sdk::signer::{EncodableKey, SeedDerivable},
...
fn main() -> Result<(), Box<dyn error::Error>> {
    let matches = app(Alembic_version::version!())
        .try_get_matches()
    ...
    match subcommand {
        ("new", matches) => {
            ...
            let outfile = if matches.is_present("outfile") {
                ...
                path.extend([".config", "Alembic", key_type.default_file_name()]);
...
mod tests {
    ...
    fn test_arguments() {
        ...
        app(Alembic_version).debug_assert();
    }
    ...
    fn test_pubkey() {
        ...
        process_test_command(&["Alembic-keygen", "pubkey", "elgamal", &keypair_path]).unwrap();
    }
}
