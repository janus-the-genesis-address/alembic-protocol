...
mod tests {
    use {
        ...
        log::*,
        Alembic_connection_cache::connection_cache_stats::ConnectionCacheStats,
        Alembic_perf::packet::PacketBatch,
        Alembic_quic_client::nonblocking::quic_client::{
            QuicClientCertificate, QuicLazyInitializedEndpoint,
        },
        Alembic_sdk::{net::DEFAULT_TPU_COALESCE, packet::PACKET_DATA_SIZE, signature::Keypair},
        Alembic_streamer::{
    ...
    fn test_quic_client_multiple_writes() {
        use {
            Alembic_connection_cache::client_connection::ClientConnection,
            Alembic_quic_client::quic_client::QuicClientConnection,
        };
        ...
        let SpawnServerResult {
            ...
            thread: t,
            key_updater: _,
        } = Alembic_streamer::quic::spawn_server(
            "solQuicTest",
            "quic_streamer_test",
            s.try_clone().unwrap(),
            &keypair,
    ...
    async fn test_nonblocking_quic_client_multiple_writes() {
        use {
            Alembic_connection_cache::nonblocking::client_connection::ClientConnection,
            Alembic_quic_client::nonblocking::quic_client::QuicClientConnection,
        };
        Alembic_logger::setup();
        let (sender, receiver) = unbounded();
        let staked_nodes = Arc::new(RwLock::new(StakedNodes::default()));
        let (s, exit, keypair) = server_args();
        let (_, _, t) = Alembic_streamer::nonblocking::quic::spawn_server(
            "quic_streamer_test",
            s.try_clone().unwrap(),
            &keypair,
            sender,
