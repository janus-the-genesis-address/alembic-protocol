...
//! ```bash
//! Alembic-dos $COMMON --num-signatures 8
//! ```
//! 1.2 With valid signatures
//! ```bash
//! Alembic-dos $COMMON --valid-signatures --num-signatures 8
//! ```
//! 2. With blockhash and payer:
//! 2.1 Single-instruction transaction
//! ```bash
//! Alembic-dos $COMMON --valid-blockhash --transaction-type transfer --num-instructions 1
//! ```
//! 2.2 Multi-instruction transaction
//! ```bash
//! Alembic-dos $COMMON --valid-blockhash --transaction-type transfer --num-instructions 8
//! ```
//! 2.3 Account-creation transaction
//! ```bash
//! Alembic-dos $COMMON --valid-blockhash --transaction-type account-creation
//! ```
//!
...
use {
    ...
    log::*,
    rand::{thread_rng, Rng},
    Alembic_bench_tps::{bench::generate_and_fund_keypairs, bench_tps_client::BenchTpsClient},
    Alembic_client::{
        connection_cache::ConnectionCache, tpu_client::TpuClientWrapper,
        tpu_connection::TpuConnection,
    },
    Alembic_core::repair::serve_repair::{RepairProtocol, RepairRequestHeader, ServeRepair},
    Alembic_dos::cli::*,
    Alembic_gossip::{
...
fn get_target(
    ...
    if nodes.is_empty() {
        // skip-gossip case
        target = Some((Alembic_sdk::pubkey::new_rand(), entrypoint_addr));
    } else {
        info!("************ NODE ***********");
        for node in nodes {
            info!("{:?}", node);
...
pub mod test {
    use {
        super::*,
        Alembic_client::tpu_client::QuicTpuClient,
        Alembic_core::validator::ValidatorConfig,
        Alembic_faucet::faucet::run_local_faucet,
        Alembic_gossip::contact_info::LegacyContactInfo,
        Alembic_local_cluster::{
            cluster::Cluster,
            local_cluster::{ClusterConfig, LocalCluster},
            validator_configs::make_identical_validator_configs,
        },
        Alembic_rpc::rpc::JsonRpcConfig,
