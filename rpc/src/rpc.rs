//! The `rpc` module implements the Alembic RPC interface.
use {
    crate::{
    ...
    Alembic_client::connection_cache::{ConnectionCache, Protocol},
    Alembic_entry::entry::Entry,
    Alembic_faucet::faucet::request_airdrop_transaction,
    Alembic_gossip::{cluster_info::ClusterInfo, contact_info::ContactInfo},
    Alembic_ledger::{
    ...
    Alembic_metrics::inc_new_counter_info,
    Alembic_perf::packet::PACKET_DATA_SIZE,
    ...
    Alembic_send_transaction_service::{
        send_transaction_service::{SendTransactionService, TransactionInfo},
        tpu_info::NullTpuInfo,
    },
    Alembic_stake_program,
    Alembic_storage_bigtable::Error as StorageError,
    ...
    Alembic_vote_program::vote_state::{VoteState, MAX_LOCKOUT_HISTORY},
    spl_token_2022::{
        extension::StateWithExtensions,
        Alembic_program::program_pack::Pack,
...
impl Default for RpcBigtableConfig {
    fn default() -> Self {
        let bigtable_instance_name = Alembic_storage_bigtable::DEFAULT_INSTANCE_NAME.to_string();
        let bigtable_app_profile_id = Alembic_storage_bigtable::DEFAULT_APP_PROFILE_ID.to_string();
        Self {
            enable_bigtable_ledger_upload: false,
            bigtable_instance_name,
            bigtable_app_profile_id,
            timeout: None,
            max_message_size: Alembic_storage_bigtable::DEFAULT_MAX_MESSAGE_SIZE,
        }
    }
}
...
pub struct JsonRpcRequestProcessor {
    ...
    bigtable_ledger_storage: Option<Alembic_storage_bigtable::LedgerStorage>,
...
impl JsonRpcRequestProcessor {
    ...
    fn bank(&self, commitment: Option<CommitmentConfig>) -> Arc<Bank> {
        ...
        r_bank_forks.get(slot).unwrap_or_else(|| {
            ...
            // a slot.
            //
            // For more information, see https://github.com/Alembic-labs/Alembic/issues/11078
            warn!(
    ...
    pub fn new_from_bank(
        ...
        let cluster_info = Arc::new({
            ...
            let contact_info = ContactInfo::new_localhost(
                &keypair.pubkey(),
                Alembic_sdk::timing::timestamp(), // wallclock
            );
    ...
    fn check_bigtable_result<T>(
        &self,
        result: &std::result::Result<T, Alembic_storage_bigtable::Error>,
    ) -> Result<()> {
        if let Err(Alembic_storage_bigtable::Error::BlockNotFound(slot)) = result {
    ...
    pub fn get_confirmed_signatures_for_address(
        ...
        if self.config.enable_rpc_transaction_history {
            ...
            // https://github.com/Alembic-labs/Alembic/pull/10928
            let end_slot = min(
                end_slot,
    ...
    pub fn get_stake_activation(
        ...
        let stake_history =
            Alembic_sdk::account::from_account::<StakeHistory, _>(&stake_history_account)
                .ok_or_else(Error::internal_error)?;
    ...
    fn get_stake_minimum_delegation(&self, config: RpcContextConfig) -> Result<RpcResponse<u64>> {
        ...
        let stake_minimum_delegation =
            Alembic_stake_program::get_minimum_delegation(&bank.feature_set);
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `Alembic-validator wait-for-restart-window` to not require this method, so
    ...
    impl Minimal for MinimalImpl {
        ...
        fn get_version(&self, _: Self::Metadata) -> Result<RpcVersionInfo> {
            ...
            let version = Alembic_version::Version::default();
            Ok(RpcVersionInfo {
                Alembic_core: version.to_string(),
            ...
            Ok(meta
                ...
                .map(|leader_schedule| {
                    let mut schedule_by_identity =
                        Alembic_ledger::leader_schedule_utils::leader_schedule_by_identity(
    ...
    impl BankData for BankDataImpl {
        ...
        fn get_block_production(
            ...
            for identity in slot_leaders {
                ...
                if slot_history.check(slot) == Alembic_sdk::slot_history::Check::Found {
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        // See https://github.com/Alembic-labs/Alembic-program-library/releases/tag/token-v2.0.0 for
        // program details
...
pub mod rpc_full {
    use {
        super::*,
        Alembic_sdk::message::{SanitizedVersionedMessage, VersionedMessage},
        Alembic_transaction_status::UiInnerInstructions,
    };
    #[rpc]
    pub trait Full {
        type Metadata;
...
pub fn create_test_transaction_entries(
    ...
    let success_tx = Alembic_sdk::system_transaction::transfer(
        mint_keypair,
        &keypair1.pubkey(),
        rent_exempt_amount,
        blockhash,
    ...
    let entry_1 = Alembic_entry::entry::next_entry(&blockhash, 1, vec![success_tx]);
    // Failed transaction, InstructionError
    let ix_error_tx = Alembic_sdk::system_transaction::transfer(
        keypair2,
        &keypair3.pubkey(),
    ...
    let entry_2 = Alembic_entry::entry::next_entry(&entry_1.hash, 1, vec![ix_error_tx]);
...
pub fn populate_blockstore_for_tests(
    ...
    let shreds = Alembic_ledger::blockstore::entries_to_test_shreds(
        &entries,
        slot,
        parent_slot,
        true,
    ...
    assert_eq!(
        Alembic_ledger::blockstore_processor::process_entries_for_tests(
            &BankWithScheduler::new_without_scheduler(bank),
            entries,
            Some(
                &Alembic_ledger::blockstore_processor::TransactionStatusSender {
...
pub mod tests {
    use {
        ...
        Alembic_accounts_db::{inline_spl_token, inline_spl_token_2022},
        Alembic_entry::entry::next_versioned_entry,
        Alembic_gossip::socketaddr,
        Alembic_ledger::{
        ...
        spl_token_2022::{
            ...
            Alembic_program::{program_option::COption, pubkey::Pubkey as SplTokenPubkey},
    ...
    impl RpcHandler {
        ...
        fn store_vote_account(&self, vote_pubkey: &Pubkey, vote_state: VoteState) {
            ...
            let mut vote_account =
                AccountSharedData::new(balance, space, &Alembic_vote_program::id());
    ...
    fn test_rpc_request_processor_new() {
        let bob_pubkey = Alembic_sdk::pubkey::new_rand();
    ...
    fn test_rpc_simulate_transaction_with_inner_instructions() {
        ...
        let lookup_table_space = Alembic_sdk::address_lookup_table::state::LOOKUP_TABLE_META_SIZE;
        ...
        let (instruction, lookup_table_address) =
            Alembic_sdk::address_lookup_table::instruction::create_lookup_table(
    ...
    fn test_rpc_get_version() {
        ...
        let expected = {
            let version = Alembic_version::Version::default();
            json!({
                "Alembic-core": version.to_string(),
    ...
    fn test_get_vote_accounts() {
        ...
        let credits_per_slot =
            Alembic_vote_program::vote_state::VOTE_CREDITS_MAXIMUM_PER_SLOT as u64;
    ...
    #[test]
    fn test_token_rpcs() {
        for program_id in Alembic_account_decoder::parse_token::spl_token_ids() {
            let rpc = RpcHandler::start();
            let bank = rpc.working_bank();
            ...
            let token_account_pubkey = Alembic_sdk::pubkey::new_rand();
            let token_with_different_mint_pubkey = Alembic_sdk::pubkey::new_rand();
            ...
            if program_id == inline_spl_token_2022::id() {
                ...
                let other_token_account_pubkey = Alembic_sdk::pubkey::new_rand();
            ...
            let token_with_smaller_balance = Alembic_sdk::pubkey::new_rand();
    ...
    fn test_decode_and_deserialize_too_large_payloads_fail() {
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base58 encoded Alembic_sdk::transaction::Transaction too large: {tx58_len} bytes (max: encoded/raw {MAX_BASE58_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64, TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base64 encoded Alembic_sdk::transaction::Transaction too large: {tx64_len} bytes (max: encoded/raw {MAX_BASE64_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "decoded Alembic_sdk::transaction::Transaction too large: {too_big} bytes (max: {PACKET_DATA_SIZE} bytes)"
            ))
        );
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64.clone(), TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(
                "failed to deserialize Alembic_sdk::transaction::Transaction: invalid value: \
